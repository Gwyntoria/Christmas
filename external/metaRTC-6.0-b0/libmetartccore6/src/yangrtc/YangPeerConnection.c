...
int32_t g_yang_pc_connectServer(YangPeer* peer) {
    ...
    if (mediaServer == Yang_Server_Zlm)
        return yang_zlm_connectRtcServer(conn);
...
void g_yang_pc_parseHeader(YangVideoCodec codec, uint8_t* buf, uint8_t* src, int32_t* hLen) {
    if (codec == Yang_VED_264)
        yang_getH264RtmpHeader(buf, src, hLen);
    if (codec == Yang_VED_265)
        yang_getH265RtmpHeader(buf, src, hLen);
}
...
void yang_destroy_peerConnection(YangPeerConnection* peerconn) {
    ...
    if (conn->session->context.state)
        g_yang_pc_disconnectServer(&peerconn->peer);
