...
void g_yang_doTask(int32_t taskId, void* user) {
    ...
    if (session->startRecv && session->play) {
        ...
        if (taskId == 1) {
            ...
            if (session->context.streamConfig->streamOptType == Yang_Stream_Play &&
                session->play->send_rtcp_xr_rrtr(&session->context, session->play->playStream))
    ...
    if (session->push) {
        if (taskId == 1) {
            ...
            if (session->play && session->context.stats.recvStats.video.rtt > 0)
                session->play->update_rtt(&session->context, session->play->playStream, session->play->playStream->videoTrack->session.track.ssrc,
...
void yang_rtcconn_do_request_keyframe(YangRtcSession* session, uint32_t ssrc) {
    if (session->context.streamConfig && session->context.streamConfig->rtcCallback.sendRequest)
        session->context.streamConfig->rtcCallback.sendRequest(session->context.streamConfig->rtcCallback.context, session->context.streamConfig->uid, ssrc,
...
void yang_rtcconn_setSsrc(YangRtcSession* session, uint32_t audioSsrc, uint32_t videoSsrc) {
#if Yang_Enable_RTC_Audio
    if (session->pushAudio)
...
int32_t yang_rtcconn_on_rtcp(YangRtcSession* session, char* data, int32_t nb_data) {
    ...
    for (int i = 0; i < session->rtcp_compound.rtcpVector.vsize; i++) {
        ...
        if (Yang_Ok != err) {
            ...
            return yang_error_wrap(err, "cipher=%u, plaintext=%u,  rtcp=(%u,%u,%u,%u)", nb_data, nb_unprotected_buf, rtcp->nb_data, rtcp->header.rc,
...
int32_t yang_rtcconn_onVideo(YangRtcSession* session, YangFrame* p) {
    ...
    if (session == NULL || session->context.dtls->session.state != YangDtlsStateClientDone)
        return Yang_Ok;
#else
    if (session == NULL || !session->context.state)
        return Yang_Ok;
#endif
...
#if Yang_Enable_Dtls
#if Yang_Enable_Datachannel
    if (session == NULL || session->context.dtls->session.state != YangDtlsStateClientDone || session->context.dtls->session.isStop == 1 ||
...
void yang_rtcconn_close(YangRtcSession* session) {
    ...
    if (yang_rtcconn_isAlive(session)) {
        if (session->context.dtls && !session->context.dtls->session.isStop)
...
#if Yang_Enable_RTC_Video
    if (session == NULL || session->play == NULL || session->play->playStream->videoTrack == NULL)
        return 1;
    YangPlayTrackVideo* videotrack = session->play->playStream->videoTrack;
    if (mess == YangRTC_Decoder_Error) {
        if (videotrack)
            videotrack->setRequestKeyframeState(&videotrack->session, yangfalse);
...
void yang_rtcconn_receive(YangRtcSession* session, char* data, int32_t size) {
    ...
    if (bt > 127 && bt < 192 && size > 12) {
        ...
        session->startRecv = 1;
        if (session->play)
            session->play->on_rtp(&session->context, session->play->playStream, data, size);
        return;
    }
    // is stun
...
client_sucess:
    ...
    session->context.state = 1;
    if (session->context.streamConfig && session->context.streamConfig->rtcCallback.sendRequest)
        session->context.streamConfig->rtcCallback.sendRequest(session->context.streamConfig->rtcCallback.context, session->context.streamConfig->uid, 0,
                                                               Yang_Req_Connected);
    return;
}
...
int32_t yang_rtcconn_startRtc(YangRtcSession* session, char* sdp) {
    ...
    if (session->context.avinfo->sys.mediaServer == Yang_Server_P2p) {
        if (session->context.avinfo->rtc.iceUsingLocalIp == 0 &&
            session->ice.iceHandle(&session->ice.session, session, yang_rtcconn_turn_receive, session->context.streamConfig->remoteIp,
                                   session->context.streamConfig->remotePort) == YangIceTurn) {
            yang_create_rtcsocket(session->context.sock, session->context.avinfo->sys.familyType, Yang_Socket_Protocol_Udp,
    ...
    yang_create_rtcsocket(session->context.sock, session->context.avinfo->sys.familyType, (YangSocketProtocol)session->context.avinfo->rtc.rtcSocketProtocol,
                          session->context.streamConfig->localPort);
...
#endif
    session->context.sock->updateRemoteAddress(&session->context.sock->session, session->context.streamConfig->remoteIp,
                                               session->isServer ? 0 : session->context.streamConfig->remotePort);
    yang_rtcconn_startudp(session);
