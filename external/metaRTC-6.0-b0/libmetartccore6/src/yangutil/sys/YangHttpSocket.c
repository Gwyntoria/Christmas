...
int32_t yang_http_post(YangIpFamilyType familyType, char* rets, char* ip, int32_t port, char* api, uint8_t* data, int32_t plen) {

    yang_socket_t socketfd = -1;
    YangIpAddress serverAddress;
    // printf("ip: %s, port: %d, familyType: %d\n", ip, port, familyType);

    char* ipAddr[16];
    yang_getIp(familyType, ip, ipAddr);

    printf("\n========== WebRTC Server ==========\nDomain [%s], IP [%s], Port [%d]\n", ip, ipAddr, port);

    yang_addr_set(&serverAddress, ipAddr, port, familyType, Yang_Socket_Protocol_Tcp);
    ...
    while (yangtrue) {
        ...
        if (nBytes > 0) {
            ...
            recvLen += nBytes;
            if (recvtimes == 0 && yang_strstr(buf, "HTTP") == NULL)
                break;
            recvtimes++;
            if (contentLen == 0) {
                p = yang_strstr(rets, Yang_Http_Content);
                if (recvtimes > 0 && p == NULL)
                    break;
                if (p == NULL)
                    continue;
            ...
            if (recvLen >= headerLen + contentLen)
                goto success;
            continue;
