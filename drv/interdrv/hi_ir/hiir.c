...
#define IR_ENABLE   ((uintptr_t)pIrRegBase + 0x00)
#define IR_CONFIG   ((uintptr_t)pIrRegBase + 0x04)
#define CNT_LEADS   ((uintptr_t)pIrRegBase + 0x08)
#define CNT_LEADE   ((uintptr_t)pIrRegBase + 0x0c)
#define CNT_SLEADE  ((uintptr_t)pIrRegBase + 0x10)
#define CNT0_B      ((uintptr_t)pIrRegBase + 0x14)
#define CNT1_B      ((uintptr_t)pIrRegBase + 0x18)
#define IR_BUSY     ((uintptr_t)pIrRegBase + 0x1c)
#define IR_DATAH    ((uintptr_t)pIrRegBase + 0x20)
#define IR_DATAL    ((uintptr_t)pIrRegBase + 0x24)
#define IR_INTM     ((uintptr_t)pIrRegBase + 0x28)
#define IR_INTS     ((uintptr_t)pIrRegBase + 0x2c)
#define IR_INTC     ((uintptr_t)pIrRegBase + 0x30)
#define IR_START    ((uintptr_t)pIrRegBase + 0x34)
...
static void hiir_enable_clock(void)
{
    void *ir_crg_addr;

    ir_crg_addr = osal_ioremap(IR_CRG_ADDR, (unsigned long)0x4);

    write_reg32(0x1 << 11, 0x1 << 11, ir_crg_addr);
...
static void hiir_disable_clock(void)
{
    ...
    write_reg32(0x0 << 11, 0x1 << 11, ir_crg_addr);
...
static long hiir_ioctl(unsigned int cmd, unsigned long arg, void *private_data)
{
    ...
    switch(cmd)
    {
        ...
        case IR_IOC_GET_CONFIG:
            #if 1
            if(osal_copy_to_user((void *)(uintptr_t)arg, &(hiir_dev.dev_parm), sizeof(hiir_dev_param)))
            {
