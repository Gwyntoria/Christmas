...
combo_dev_attr_t MIPI_2lane_CHN0_SENSOR_GC2053_10BIT_2M_WDR2TO1_ATTR =
{
    .devno = 0,
    .input_mode = INPUT_MODE_MIPI,
    .data_rate = MIPI_DATA_RATE_X1,
...
combo_dev_attr_t MIPI_2lane_CHN1_SENSOR_GC2053_10BIT_2M_WDR2TO1_ATTR =
{
    .devno = 1,
    .input_mode = INPUT_MODE_MIPI,
    .data_rate = MIPI_DATA_RATE_X1,
...
HI_S32 SAMPLE_COMM_VI_GetComboAttrBySns(SAMPLE_SNS_TYPE_E enSnsType, combo_dev_t MipiDev, combo_dev_attr_t* pstComboAttr)
{
    switch (enSnsType)
    {
        ...
        case GALAXYCORE_GC2053_MIPI_2M_30FPS_10BIT_WDR2TO1:
            if (0 == MipiDev)
            {
                memcpy_s(pstComboAttr, sizeof(combo_dev_attr_t), &MIPI_2lane_CHN0_SENSOR_GC2053_10BIT_2M_WDR2TO1_ATTR, sizeof(combo_dev_attr_t));
            }
            else if (1 == MipiDev)
            {
                memcpy_s(pstComboAttr, sizeof(combo_dev_attr_t), &MIPI_2lane_CHN1_SENSOR_GC2053_10BIT_2M_WDR2TO1_ATTR, sizeof(combo_dev_attr_t));
            }
            break;
...
/**
 * @brief Divide input lane, if there is more than one input source, lan_divide_mode must be set as LANE_DIVIDE_MODE_1
 * 
 * @param pstViConfig VI configure
 * @return HI_U32 lane divide mode number
 */
...
//         || (SONY_IMX274_MIPI_2M_30FPS_12BIT_WDR2TO1 == pstViConfig->astViInfo[0].stSnsInfo.enSnsType))
    if ((GALAXYCORE_GC2053_MIPI_2M_30FPS_10BIT == pstViConfig->astViInfo[0].stSnsInfo.enSnsType
            || GALAXYCORE_GC2053_MIPI_2M_30FPS_10BIT_WDR2TO1 == pstViConfig->astViInfo[0].stSnsInfo.enSnsType))
    {
...
HI_S32 SAMPLE_COMM_VI_SetParam(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    ...
    for (i = 0; i < pstViConfig->s32WorkingViNum; i++)
    {
        ...
        printf("i = %d s32ViNum=%d  ViPipe=%d\n",i, s32ViNum, ViPipe);
        printf("pstViInfo->stPipeInfo.bMultiPipe = %d\n",pstViInfo->stPipeInfo.bMultiPipe);
...
HI_S32 SAMPLE_COMM_VI_StartIsp(SAMPLE_VI_INFO_S* pstViInfo)
{
    ...
    for (i = 0; i < WDR_MAX_PIPE_NUM; i++)
    {
        ...
        if (pstViInfo->stPipeInfo.aPipe[i] >= 0  && pstViInfo->stPipeInfo.aPipe[i] < VI_MAX_PIPE_NUM)
        {
            ...
            printf("stPubAttr.enWDRMode=%d  ViPipe=%d u32SnsId=%d\n ",stPubAttr.enWDRMode, ViPipe, u32SnsId);
