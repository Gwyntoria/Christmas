...
ISP_PUB_ATTR_S ISP_PUB_ATTR_IMX385_2M_60FPS =
{
    {0, 0, 1920, 1080},
    {1920, 1080},
    // 60,
...
ISP_SNS_OBJ_S* LOTO_COMM_ISP_GetSnsObj(HI_U32 u32SnsId)
{
...
static void* LOTO_COMM_ISP_Thread(void* param)
{
    HI_S32 s32Ret;
    ISP_DEV IspDev;
    ...
    LOGD("ISP Dev %d running !\n", IspDev);
    s32Ret = HI_MPI_ISP_Run(IspDev);

    if (HI_SUCCESS != s32Ret)
    {
        LOGE("HI_MPI_ISP_Run failed with %#x!\n", s32Ret);
    }
...
HI_S32 LOTO_COMM_ISP_Run(ISP_DEV IspDev)
{
    ...
    s32Ret = pthread_create(&g_IspPid[IspDev], pstAttr, LOTO_COMM_ISP_Thread, (HI_VOID*)IspDev);

    if (0 != s32Ret)
    {
        LOGE("create isp running thread failed!, error: %d, %s\r\n", s32Ret, strerror(s32Ret));
        goto out;
    }
...
HI_S32 LOTO_COMM_ISP_Sensor_Regiter_callback(ISP_DEV IspDev, HI_U32 u32SnsId)
{
    ...
    {
        LOGE("invalid sensor id: %d\n", u32SnsId);
        return HI_FAILURE;
    }

    pstSnsObj = LOTO_COMM_ISP_GetSnsObj(u32SnsId);

    if (HI_NULL == pstSnsObj)
    {
        LOGE("sensor %d not exist!\n", u32SnsId);
        return HI_FAILURE;
    }
  ...
  //  strncpy(stAfLib.acLibName, HI_AF_LIB_NAME, sizeof(HI_AF_LIB_NAME));
    ...
    if (pstSnsObj->pfnRegisterCallback != HI_NULL)
    {
        ...
        {
            LOGE("sensor_register_callback failed with %#x!\n", s32Ret);
            return s32Ret;
        }
    }
    else
    {
        LOGE("sensor_register_callback failed with HI_NULL!\n");
    }
...
HI_S32 LOTO_COMM_ISP_Sensor_UnRegiter_callback(ISP_DEV IspDev)
{
 ...
 //   strncpy(stAfLib.acLibName, HI_AF_LIB_NAME, sizeof(HI_AF_LIB_NAME));
    ...
    if (pstSnsObj->pfnUnRegisterCallback != HI_NULL)
    {
        ...
        if (s32Ret != HI_SUCCESS)
        {
            LOGE("sensor_unregister_callback failed with %#x!\n", s32Ret);
            return s32Ret;
        }
    }
    else
    {
        LOGE("sensor_unregister_callback failed with HI_NULL!\n");
    }
...
HI_VOID LOTO_COMM_ISP_Stop(ISP_DEV IspDev)
{
    if (g_IspPid[IspDev])
    {
        ...
        LOTO_COMM_ISP_Awblib_UnCallback(IspDev);
        LOTO_COMM_ISP_Aelib_UnCallback(IspDev);
        LOTO_COMM_ISP_Sensor_UnRegiter_callback(IspDev);
        g_IspPid[IspDev] = 0;
    }
...
HI_VOID LOTO_COMM_All_ISP_Stop(HI_VOID)
{
    ...
    {
        LOTO_COMM_ISP_Stop(IspDev);
    }
}

static ISP_SNS_TYPE_E LOTO_COMM_GetSnsBusType(SAMPLE_SNS_TYPE_E enSnsType)
{
...
HI_S32 LOTO_COMM_ISP_BindSns(ISP_DEV IspDev, HI_U32 u32SnsId, SAMPLE_SNS_TYPE_E enSnsType, HI_S8 s8SnsDev)
{
    ...
    enBusType = LOTO_COMM_GetSnsBusType(enSnsType);
    ...
    if (HI_NULL != pstSnsObj->pfnSetBusInfo)
    {
        ...
        if (s32Ret != HI_SUCCESS)
        {
            LOGE("set sensor bus info failed with %#x!\n", s32Ret);
            return s32Ret;
        }
    }
    else
    {
        LOGE("not support set sensor bus info!\n");
        return HI_FAILURE;
    }
