...
HI_S32 SAMPLE_Tlv320_CfgAudio(AIO_MODE_E enWorkmode, AUDIO_SAMPLE_RATE_E enSample)
{
    ...
    audio_ctrl.if_powerup = 1;
    LOGE("audio_ctrl.input_level=0x%x,audio_ctrl.if_powerup=0x%x\n",audio_ctrl.input_level,audio_ctrl.if_powerup);
...
//     if (HI_SUCCESS != s32Ret)
//     {
//         LOGE("%s: Get ai chn param failed\n", __FUNCTION__);
//         return NULL;
//     }
...
//     if (HI_SUCCESS != s32Ret)
//     {
//         LOGE("%s: set ai chn param failed\n", __FUNCTION__);
//         return NULL;
//     }
...
//         else if (0 == s32Ret)
//         {
//             LOGE("%s: get ai frame select time out\n", __FUNCTION__);
//             break;
//         }
...
//             {
// #if 0
//                 LOGE("%s: HI_MPI_AI_GetFrame(%d, %d), failed with %#x!\n", \
...
//                 if (HI_SUCCESS != s32Ret )
//                 {
//                     LOGE("%s: HI_MPI_AENC_SendFrame(%d), failed with %#x!\n", \
...
//                 if (HI_SUCCESS != s32Ret )
//                 {
//                     LOGE("%s: HI_MPI_AO_SendFrame(%d, %d), failed with %#x!\n", \
...
//             if (HI_SUCCESS != s32Ret )
//             {
//                 LOGE("%s: HI_MPI_AI_ReleaseFrame(%d, %d), failed with %#x!\n", \
//                        __FUNCTION__, pstAiCtl->AiDev, pstAiCtl->AiChn, s32Ret);
//                 pstAiCtl->bStart = HI_FALSE;
//                 return NULL;
//             }
...
//         else if (0 == s32Ret)
//         {
//             LOGE("%s: get aenc stream select time out\n", __FUNCTION__);
//             break;
//         }
...
//             //LOGE("LOTO_COMM_AUDIO_AencProc- Time: %lu \n", timestamp);
//             s32Ret = HI_MPI_AENC_GetStream(pstAencCtl->AeChn, &stStream, HI_FALSE);
//             if (HI_SUCCESS != s32Ret )
//             {
//                 LOGE("%s: HI_MPI_AENC_GetStream(%d), failed with %#x!\n", \
...
//             //     if (HI_SUCCESS != s32Ret )
//             //     {
//             //         LOGE("%s: HI_MPI_ADEC_SendStream(%d), failed with %#x!\n", \
...
//             if (HI_SUCCESS != s32Ret )
//             {
//                 LOGE("%s: HI_MPI_AENC_ReleaseStream(%d), failed with %#x!\n", \
//                        __FUNCTION__, pstAencCtl->AeChn, s32Ret);
//                 pstAencCtl->bStart = HI_FALSE;
//                 return NULL;
//             }
...
//             if (HI_SUCCESS != s32Ret)
//             {
//                 LOGE("%s: HI_MPI_ADEC_SendEndOfStream failed!\n", __FUNCTION__);
//             }
...
//             if (HI_SUCCESS != s32Ret)
//             {
//                 LOGE("%s: HI_MPI_AO_SetVolume(%d), failed with %#x!\n", \
//                        __FUNCTION__, AoDev, s32Ret);
//             }
//             LOGE("\rset volume %d          ", s32Volume);
//             fflush(stdout);
//             sleep(2);
//         }
...
//     pthread_create(&pstAi->stAiPid, 0, LOTO_COMM_AUDIO_AiProc, pstAi);
...
//     LOGE("LOTO_COMM_AUDIO_CreatTrdAencAdec \n");
...
//     pthread_create(&pstAdec->stAdPid, 0, , pstAdec);
...
//     pthread_create(&pstAoCtl->stAoPid, 0, LOTO_COMM_AUDIO_AoVolProc, pstAoCtl);
...
HI_S32 LOTO_COMM_AUDIO_StopHdmi(HI_VOID)
{
    HI_S32 s32Ret;
    VO_DEV VoDev = 0;
...
//         LOTO_COMM_AUDIO_StartHdmi(pstAioAttr);
// #endif
//     }
...
//     if (HI_SUCCESS != s32Ret)
//     {
//         LOGE("%s: HI_MPI_AO_SetPubAttr(%d) failed with %#x!\n", __FUNCTION__, \
//                AoDevId, s32Ret);
//         return HI_FAILURE;
//     }
...
//     if (HI_SUCCESS != s32Ret)
//     {
//         LOGE("%s: HI_MPI_AO_Enable(%d) failed with %#x!\n", __FUNCTION__, AoDevId, s32Ret);
//         return HI_FAILURE;
//     }
...
//         if (HI_SUCCESS != s32Ret)
//         {
//             LOGE("%s: HI_MPI_AO_EnableChn(%d) failed with %#x!\n", __FUNCTION__, i, s32Ret);
//             return HI_FAILURE;
//         }
...
//             if (HI_SUCCESS != s32Ret)
//             {
//                 LOGE("%s: HI_MPI_AO_EnableReSmp(%d,%d) failed with %#x!\n", __FUNCTION__, AoDevId, i, s32Ret);
//                 return HI_FAILURE;
//             }
//         }
...
//             if (HI_SUCCESS != s32Ret)
//             {
//                 LOGE("%s: HI_MPI_AO_DisableReSmp failed with %#x!\n", __FUNCTION__, s32Ret);
//                 return s32Ret;
//             }
//         }
...
//         if (HI_SUCCESS != s32Ret)
//         {
//             LOGE("%s: HI_MPI_AO_DisableChn failed with %#x!\n", __FUNCTION__, s32Ret);
//             return s32Ret;
//         }
//     }
...
//     if (HI_SUCCESS != s32Ret)
//     {
//         LOGE("%s: HI_MPI_AO_Disable failed with %#x!\n", __FUNCTION__, s32Ret);
//         return s32Ret;
//     }
...
// #ifdef HI_ACODEC_TYPE_HDMI
//         s32Ret = LOTO_COMM_AUDIO_StopHdmi();
//         if (HI_SUCCESS != s32Ret)
//         {
//             LOGE("%s: LOTO_COMM_AUDIO_StopHdmi failed with %#x!\n", __FUNCTION__, s32Ret);
//             return s32Ret;
//         }
...
HI_S32 LOTO_COMM_AUDIO_StartAenc(HI_S32 s32AencChnCnt, AIO_ATTR_S *pstAioAttr, PAYLOAD_TYPE_E enType)
{
    ...
    if (PT_ADPCMA == stAencAttr.enType) {
        ...
        // stAencOpus.application = OPUS_APPLICATION_AUDIO;
...
//     if (HI_SUCCESS != s32Ret)
//     {
//         LOGE("%s: HI_MPI_ADEC_CreateChn(%d) failed with %#x!\n", __FUNCTION__, \
//                AdChn, s32Ret);
//         return s32Ret;
//     }
//     return 0;
...
//     if (HI_SUCCESS != s32Ret)
//     {
//         LOGE("%s: HI_MPI_ADEC_DestroyChn(%d) failed with %#x!\n", __FUNCTION__,
//                AdChn, s32Ret);
//         return s32Ret;
//     }
