...
HI_S32 LOTO_COMM_SYS_MemConfig(HI_VOID)
{
    ...
    for (i = 0; i < VI_MAX_PIPE_NUM; i++)
    {
        for (j = 0; j < VI_MAX_CHN_NUM; j++)
        {
            ...
            if (s32Ret)
            {
                LOGE("HI_MPI_SYS_SetMemConfig ERR !\n");
                return HI_FAILURE;
            }
        }
    }
    ...
    for (i = 0; i < VENC_MAX_CHN_NUM; i++)
    {
        ...
        if (s32Ret)
        {
            LOGE("HI_MPI_SYS_SetMemConf ERR !\n");
            return HI_FAILURE;
        }
    }
...
HI_S32 LOTO_COMM_SYS_Init(VB_CONFIG_S* pstVbConfig)
{
    ...
    if (NULL == pstVbConfig)
    {
        LOGE("input parameter is null, it is invaild!\n");
        return HI_FAILURE;
    }
    ...
    {
        LOGE("HI_MPI_VB_SetConf failed, s32Ret = %#x!\n", s32Ret);
        return HI_FAILURE;
    }
...
HI_S32 LOTO_COMM_SYS_InitWithVbSupplement(VB_CONFIG_S* pstVbConf, HI_U32 u32SupplementConfig)
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
        LOGE("HI_MPI_VB_SetSupplementConf failed!\n");
        return HI_FAILURE;
    }
