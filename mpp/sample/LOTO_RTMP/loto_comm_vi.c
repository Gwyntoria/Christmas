...
HI_S32 LOTO_COMM_VI_ResetSns(sns_rst_source_t SnsDev)
{
    HI_S32 fd;
    HI_S32 s32Ret;
    ...
    {
        LOGE("HI_MIPI_SET_HS_MODE failed\n");
    }
...
HI_S32 LOTO_COMM_VI_SetMipiHsMode(lane_divide_mode_t enHsMode)
{
    HI_S32 fd;
    HI_S32 s32Ret;
...
HI_S32 LOTO_COMM_VI_EnableMipiClock(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    HI_S32              i = 0;
    HI_S32              s32ViNum = 0;
    ...
    for (i = 0; i < pstViConfig->s32WorkingViNum; i++)
    {
        ...
        if (HI_SUCCESS != s32Ret)
        {
            LOGE("MIPI_ENABLE_CLOCK %d failed\n", devno);
            goto EXIT;
        }
    }
...
HI_S32 LOTO_COMM_VI_DisableMipiClock(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    HI_S32              i = 0;
    HI_S32              s32ViNum = 0;
    HI_S32              s32Ret = HI_SUCCESS;
    ...
    for (i = 0; i < pstViConfig->s32WorkingViNum; i++)
    {
        ...
        if (HI_SUCCESS != s32Ret)
        {
            LOGE("MIPI_DISABLE_CLOCK %d failed\n", devno);
            goto EXIT;
        }
    }
...
HI_S32 LOTO_COMM_VI_ResetMipi(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    HI_S32              i = 0;
    HI_S32              s32ViNum = 0;
    HI_S32              s32Ret = HI_SUCCESS;
...
HI_S32 LOTO_COMM_VI_UnresetMipi(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    HI_S32              i = 0;
    HI_S32              s32ViNum = 0;
    HI_S32              s32Ret = HI_SUCCESS;
...
HI_S32 LOTO_COMM_VI_EnableSensorClock(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    HI_S32              s32Ret = HI_SUCCESS;
    HI_S32              fd;
    ...
    for (SnsDev = 0; SnsDev < SNS_MAX_CLK_SOURCE_NUM; SnsDev++)
    {
        ...
        if (HI_SUCCESS != s32Ret)
        {
            LOGE("HI_MIPI_ENABLE_SENSOR_CLOCK failed\n");
            goto EXIT;
        }
    }
...
HI_S32 LOTO_COMM_VI_DisableSensorClock(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    HI_S32              s32Ret = HI_SUCCESS;
    HI_S32              fd;
    sns_clk_source_t       SnsDev = 0;
    ...
    for (SnsDev = 0; SnsDev < SNS_MAX_CLK_SOURCE_NUM; SnsDev++)
    {
        ...
        if (HI_SUCCESS != s32Ret)
        {
            LOGE("HI_MIPI_DISABLE_SENSOR_CLOCK failed\n");
            goto EXIT;
        }
    }
...
HI_S32 LOTO_COMM_VI_ResetSensor(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    HI_S32              s32Ret = HI_SUCCESS;
    HI_S32              fd;
    ...
    for (SnsDev = 0; SnsDev < SNS_MAX_RST_SOURCE_NUM; SnsDev++)
    {
        ...
        if (HI_SUCCESS != s32Ret)
        {
            LOGE("HI_MIPI_RESET_SENSOR failed\n");
            goto EXIT;
        }
    }
...
HI_S32 LOTO_COMM_VI_UnresetSensor(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    HI_S32              s32Ret = HI_SUCCESS;
    HI_S32              fd;
    sns_rst_source_t       SnsDev = 0;
    ...
    for (SnsDev = 0; SnsDev < SNS_MAX_RST_SOURCE_NUM; SnsDev++)
    {
        ...
        if (HI_SUCCESS != s32Ret)
        {
            LOGE("HI_MIPI_UNRESET_SENSOR failed\n");
            goto EXIT;
        }
    }
...
HI_S32 LOTO_COMM_VI_GetComboAttrBySns(SAMPLE_SNS_TYPE_E enSnsType, combo_dev_t MipiDev, combo_dev_attr_t* pstComboAttr)
{
    switch (enSnsType)
    {
        // case SONY_IMX274_MIPI_2M_30FPS_12BIT:
        ...
        case SONY_IMX385_MIPI_2M_60FPS_12BIT:
            memcpy_s(pstComboAttr, sizeof(combo_dev_attr_t), &MIPI_4lane_CHN0_SENSOR_IMX385_12BIT_2M_NOWDR_ATTR, sizeof(combo_dev_attr_t));
            break;
        ...
        case GALAXYCORE_GC2053_MIPI_2M_30FPS_10BIT:
            ...
            {
                memcpy_s(pstComboAttr, sizeof(combo_dev_attr_t), &MIPI_2lane_CHN1_SENSOR_GC2053_10BIT_2M_NOWDR_ATTR, sizeof(combo_dev_attr_t));
            }
...
HI_S32 LOTO_COMM_VI_SetMipiAttr(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    HI_S32              i = 0;
    HI_S32              s32ViNum = 0;
    HI_S32              s32Ret = HI_SUCCESS;
    ...
    for (i = 0; i < pstViConfig->s32WorkingViNum; i++)
    {
        ...
        LOTO_COMM_VI_GetComboAttrBySns(pstViInfo->stSnsInfo.enSnsType, pstViInfo->stSnsInfo.MipiDev, &stcomboDevAttr);
        stcomboDevAttr.devno = pstViInfo->stSnsInfo.MipiDev;

        LOGD("============= MipiDev %d, SetMipiAttr enWDRMode: %d\n", pstViInfo->stSnsInfo.MipiDev, pstViInfo->stDevInfo.enWDRMode);

        s32Ret = ioctl(fd, HI_MIPI_SET_DEV_ATTR, &stcomboDevAttr);

        if (HI_SUCCESS != s32Ret)
        {
            LOGE("MIPI_SET_DEV_ATTR failed\n");
            goto EXIT;
        }
    }
...
HI_U32 LOTO_COMM_VI_GetMipiLaneDivideMode(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    lane_divide_mode_t lane_divide_mode;

//    if ((SONY_IMX274_MIPI_2M_30FPS_12BIT == pstViConfig->astViInfo[0].stSnsInfo.enSnsType)
//         || (SONY_IMX274_MIPI_2M_30FPS_12BIT_WDR2TO1 == pstViConfig->astViInfo[0].stSnsInfo.enSnsType))
    if ((GALAXYCORE_GC2053_MIPI_2M_30FPS_10BIT == pstViConfig->astViInfo[0].stSnsInfo.enSnsType))
    {
...
* function : init mipi
*****************************************************************************/
HI_S32 LOTO_COMM_VI_StartMIPI(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    HI_S32 s32Ret = HI_SUCCESS;
    //lane_divide_mode_t lane_divide_mode = LANE_DIVIDE_MODE_0;
    lane_divide_mode_t lane_divide_mode = LANE_DIVIDE_MODE_1;
    ...
    lane_divide_mode = LOTO_COMM_VI_GetMipiLaneDivideMode(pstViConfig);

    LOGD("lane_divide_mode = %d\n", lane_divide_mode);
    
    s32Ret = LOTO_COMM_VI_SetMipiHsMode(lane_divide_mode);

    if (HI_SUCCESS != s32Ret)
    {
        LOGE("LOTO_COMM_VI_SetMipiHsMode failed!\n");

        return HI_FAILURE;
    }

    s32Ret = LOTO_COMM_VI_EnableMipiClock(pstViConfig);
    if (HI_SUCCESS != s32Ret)
    {
        LOGE("LOTO_COMM_VI_EnableMipiClock failed!\n");

        return HI_FAILURE;
    }

    s32Ret = LOTO_COMM_VI_ResetMipi(pstViConfig);

    if (HI_SUCCESS != s32Ret)
    {
        LOGE("LOTO_COMM_VI_ResetMipi failed!\n");

        return HI_FAILURE;
    }

    s32Ret = LOTO_COMM_VI_EnableSensorClock(pstViConfig);
    if (HI_SUCCESS != s32Ret)
    {
        LOGE("LOTO_COMM_VI_EnableSensorClock failed!\n");

        return HI_FAILURE;
    }

    s32Ret = LOTO_COMM_VI_ResetSensor(pstViConfig);

    if (HI_SUCCESS != s32Ret)
    {
        LOGE("LOTO_COMM_VI_ResetSensor failed!\n");

        return HI_FAILURE;
    }

    s32Ret = LOTO_COMM_VI_SetMipiAttr(pstViConfig);

    if (HI_SUCCESS != s32Ret)
    {
        LOGE("LOTO_COMM_VI_SetMipiAttr failed!\n");

        return HI_FAILURE;
    }

    s32Ret = LOTO_COMM_VI_UnresetMipi(pstViConfig);

    if (HI_SUCCESS != s32Ret)
    {
        LOGE("LOTO_COMM_VI_UnresetMipi failed!\n");

        return HI_FAILURE;
    }

    s32Ret = LOTO_COMM_VI_UnresetSensor(pstViConfig);

    if (HI_SUCCESS != s32Ret)
    {
        LOGE("LOTO_COMM_VI_UnresetSensor failed!\n");
...
HI_S32 LOTO_COMM_VI_StopMIPI(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    HI_S32 s32Ret = HI_SUCCESS;
    ...
    s32Ret = LOTO_COMM_VI_DisableSensorClock(pstViConfig);
    if (HI_SUCCESS != s32Ret)
    {
        LOGE("LOTO_COMM_VI_DisableSensorClock failed!\n");
    ...
    s32Ret = LOTO_COMM_VI_DisableMipiClock(pstViConfig);
    if (HI_SUCCESS != s32Ret)
    {
        LOGE("LOTO_COMM_VI_DisableMipiClock failed!\n");
...
HI_S32 LOTO_COMM_VI_UpdateVIVPSSMode(VI_VPSS_MODE_S* pstVIVPSSMode)
{
    HI_S32   i = 0;
...
HI_S32 LOTO_COMM_VI_SetParam(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    HI_S32              i;
    HI_S32              s32ViNum;
    HI_S32              s32Ret;
    ...
    if (HI_SUCCESS != s32Ret)
    {
        LOGE("Get VI-VPSS mode Param failed with %#x!\n", s32Ret);
    ...
    for (i = 0; i < pstViConfig->s32WorkingViNum; i++)
    {
        ...
        // LOGD("i = %d s32ViNum=%d  ViPipe=%d\n", i, s32ViNum, ViPipe);
        // LOGD("pstViInfo->stPipeInfo.bMultiPipe = %d\n", pstViInfo->stPipeInfo.bMultiPipe);

        if ((pstViInfo->stPipeInfo.bMultiPipe == HI_TRUE)
            || (VI_OFFLINE_VPSS_ONLINE == pstViInfo->stPipeInfo.enMastPipeMode))
        {
            LOTO_COMM_VI_UpdateVIVPSSMode(&stVIVPSSMode);
    ...
    if (HI_SUCCESS != s32Ret)
    {
        LOGE("Set VI-VPSS mode Param failed with %#x!\n", s32Ret);
...
HI_S32 LOTO_COMM_VI_GetDevAttrBySns(SAMPLE_SNS_TYPE_E enSnsType, VI_DEV_ATTR_S* pstViDevAttr)
{
    switch (enSnsType)
    {
        // case SONY_IMX274_MIPI_2M_30FPS_12BIT:
        ...
        case SONY_IMX385_MIPI_2M_30FPS_12BIT:
        case SONY_IMX385_MIPI_2M_60FPS_12BIT:
            memcpy_s(pstViDevAttr, sizeof(VI_DEV_ATTR_S), &DEV_ATTR_IMX385_2M_BASE, sizeof(VI_DEV_ATTR_S));
...
HI_S32 LOTO_COMM_VI_GetPipeAttrBySns(SAMPLE_SNS_TYPE_E enSnsType, VI_PIPE_ATTR_S* pstPipeAttr)
{
    switch (enSnsType)
    {
        // case SONY_IMX274_MIPI_2M_30FPS_12BIT:   //zjl
        // case SONY_IMX219_MIPI_2M_30FPS_12BIT:   //zjl
        case SONY_IMX327_MIPI_2M_30FPS_12BIT:
        case SONY_IMX327_2L_MIPI_2M_30FPS_12BIT:
        case SONY_IMX327_2L_MIPI_2M_30FPS_12BIT_WDR2TO1:
        ...
        // case SONY_IMX219_MIPI_2M_30FPS_12BIT_WDR2TO1:   //zjl
        case SONY_IMX327_MIPI_2M_30FPS_12BIT_WDR2TO1:
            memcpy_s(pstPipeAttr, sizeof(VI_PIPE_ATTR_S), &PIPE_ATTR_1920x1080_RAW12_420_3DNR_RFR, sizeof(VI_PIPE_ATTR_S));
...
HI_S32 LOTO_COMM_VI_GetChnAttrBySns(SAMPLE_SNS_TYPE_E enSnsType, VI_CHN_ATTR_S* pstChnAttr)
{
    switch (enSnsType)
    {
        // case SONY_IMX274_MIPI_2M_30FPS_12BIT:
...
HI_S32 LOTO_COMM_VI_StartDev(SAMPLE_VI_INFO_S* pstViInfo)
{
    HI_S32              s32Ret;
    VI_DEV              ViDev;
    ...
    LOTO_COMM_VI_GetDevAttrBySns(enSnsType, &stViDevAttr);
    ...
    {
        LOGE("HI_MPI_VI_SetDevAttr failed with %#x!\n", s32Ret);
        return HI_FAILURE;
    }
    ...
    if (s32Ret != HI_SUCCESS)
    {
        LOGE("HI_MPI_VI_EnableDev failed with %#x!\n", s32Ret);
        return HI_FAILURE;
    }
...
HI_S32 LOTO_COMM_VI_StopDev(SAMPLE_VI_INFO_S* pstViInfo)
{
    HI_S32 s32Ret;
    VI_DEV ViDev;
    ...
    if (s32Ret != HI_SUCCESS)
    {
        LOGE("HI_MPI_VI_DisableDev failed with %#x!\n", s32Ret);
        return HI_FAILURE;
    }
...
HI_S32 LOTO_COMM_VI_BindPipeDev(SAMPLE_VI_INFO_S* pstViInfo)
{
    HI_S32              i;
    HI_S32              s32PipeCnt = 0;
    HI_S32              s32Ret;
    ...
    if (s32Ret != HI_SUCCESS)
    {
        LOGE("HI_MPI_VI_SetDevBindPipe failed with %#x!\n", s32Ret);
        return HI_FAILURE;
    }
...
static HI_S32 LOTO_COMM_VI_ModeSwitchCreateSingleViPipe(VI_PIPE ViPipe, VI_PIPE_ATTR_S* pstPipeAttr)
{
    HI_S32 s32Ret;
    ...
    if (s32Ret != HI_SUCCESS)
    {
        LOGE("HI_MPI_VI_CreatePipe failed with %#x!\n", s32Ret);
        return HI_FAILURE;
    }
...
static HI_S32 LOTO_COMM_VI_ModeSwitch_EnableSingleViPipe(VI_PIPE ViPipe, VI_PIPE_ATTR_S* pstPipeAttr)
{
    HI_S32 s32Ret;
    ...
    if (s32Ret != HI_SUCCESS)
    {
        ...
        LOGE("HI_MPI_VI_StartPipe failed with %#x!\n", s32Ret);
        return HI_FAILURE;
    }
...
static HI_S32 LOTO_COMM_VI_StopSingleViPipe(VI_PIPE ViPipe)
{
    HI_S32  s32Ret;
    ...
    if (s32Ret != HI_SUCCESS)
    {
        LOGE("HI_MPI_VI_StopPipe failed with %#x!\n", s32Ret);
        return HI_FAILURE;
    }
    ...
    if (s32Ret != HI_SUCCESS)
    {
        LOGE("HI_MPI_VI_DestroyPipe failed with %#x!\n", s32Ret);
        return HI_FAILURE;
    }
...
HI_S32 LOTO_COMM_VI_ModeSwitch_StartViPipe(SAMPLE_VI_INFO_S* pstViInfo)
{
    HI_S32          i, j;
    HI_S32          s32Ret = HI_SUCCESS;
    VI_PIPE         ViPipe;
    ...
    for (i = 0; i < WDR_MAX_PIPE_NUM; i++)
    {
        if (pstViInfo->stPipeInfo.aPipe[i] >= 0  && pstViInfo->stPipeInfo.aPipe[i] < VI_MAX_PIPE_NUM)
        {
            ...
            LOTO_COMM_VI_GetPipeAttrBySns(pstViInfo->stSnsInfo.enSnsType, &stPipeAttr);
            {
                s32Ret = LOTO_COMM_VI_ModeSwitchCreateSingleViPipe(ViPipe, &stPipeAttr);
                if (s32Ret != HI_SUCCESS)
                {
                    LOGE("LOTO_COMM_VI_StartSingleViPipe  %d failed!\n", ViPipe);
                    goto EXIT;
                }
            }
...
EXIT:
    ...
    {
        ViPipe = j;
        LOTO_COMM_VI_StopSingleViPipe(ViPipe);
    }
...
HI_S32 LOTO_COMM_VI_ModeSwitch_EnableViPipe(SAMPLE_VI_INFO_S* pstViInfo)
{
    HI_S32          i, j;
    HI_S32          s32Ret = HI_SUCCESS;
    VI_PIPE         ViPipe;
    ...
    for (i = 0; i < WDR_MAX_PIPE_NUM; i++)
    {
        if (pstViInfo->stPipeInfo.aPipe[i] >= 0  && pstViInfo->stPipeInfo.aPipe[i] < VI_MAX_PIPE_NUM)
        {
            ...
            LOTO_COMM_VI_GetPipeAttrBySns(pstViInfo->stSnsInfo.enSnsType, &stPipeAttr);
            {
                s32Ret = LOTO_COMM_VI_ModeSwitch_EnableSingleViPipe(ViPipe, &stPipeAttr);
                if (s32Ret != HI_SUCCESS)
                {
                    LOGE("LOTO_COMM_VI_StartSingleViPipe  %d failed!\n", ViPipe);
                    goto EXIT;
...
HI_S32 LOTO_COMM_VI_StartViPipe(SAMPLE_VI_INFO_S* pstViInfo)
{
    HI_S32          i, j;
    HI_S32          s32Ret = HI_SUCCESS;
    VI_PIPE         ViPipe;
        ...
        if (pstViInfo->stPipeInfo.aPipe[i] >= 0  && pstViInfo->stPipeInfo.aPipe[i] < VI_MAX_PIPE_NUM)
        {
            ...
            else
            {
                ...
                if (HI_TRUE == pstViInfo->stPipeInfo.bVcNumCfged)
                {
                    ...
                    {
                        HI_MPI_VI_DestroyPipe(ViPipe);
                        LOGE("HI_MPI_VI_SetPipeVCNumber failed with %#x!\n", s32Ret);
                        return HI_FAILURE;
                    }
                }
...
HI_S32 LOTO_COMM_VI_StopViPipe(SAMPLE_VI_INFO_S* pstViInfo)
{
    HI_S32  i;
    VI_PIPE ViPipe;
...
HI_S32 LOTO_COMM_VI_ModeSwitch_StartViChn(SAMPLE_VI_INFO_S* pstViInfo)
{
    HI_S32              i;
    HI_BOOL             bNeedChn;
    HI_S32              s32Ret = HI_SUCCESS;
    ...
    for (i = 0; i < WDR_MAX_PIPE_NUM; i++)
    {
        if (pstViInfo->stPipeInfo.aPipe[i] >= 0  && pstViInfo->stPipeInfo.aPipe[i] < VI_MAX_PIPE_NUM)
        {
            ...
            LOTO_COMM_VI_GetChnAttrBySns(pstViInfo->stSnsInfo.enSnsType, &stChnAttr);
            ...
            if (bNeedChn)
            {
                ...
                if (s32Ret != HI_SUCCESS)
                {
                    LOGE("HI_MPI_VI_SetChnAttr failed with %#x!\n", s32Ret);
                    return HI_FAILURE;
                }
            }
        }
...
HI_S32 LOTO_COMM_VI_StartViChn(SAMPLE_VI_INFO_S* pstViInfo)
{
    HI_S32              i;
    HI_BOOL             bNeedChn;
    HI_S32              s32Ret = HI_SUCCESS;
        ...
        if (pstViInfo->stPipeInfo.aPipe[i] >= 0  && pstViInfo->stPipeInfo.aPipe[i] < VI_MAX_PIPE_NUM)
        {
            ...
            if (bNeedChn)
            {
                    ...
                    || VI_PARALLEL_VPSS_OFFLINE == enMastPipeMode)
                {
                    ...
                    if (s32Ret != HI_SUCCESS)
                    {
                        LOGE("HI_MPI_VI_EnableChn failed with %#x!\n", s32Ret);
                        return HI_FAILURE;
                    }
                }
            }
...
HI_S32 LOTO_COMM_VI_StopViChn(SAMPLE_VI_INFO_S* pstViInfo)
{
    HI_S32              i;
    HI_BOOL             bNeedChn;
    HI_S32              s32Ret = HI_SUCCESS;
        ...
        if (pstViInfo->stPipeInfo.aPipe[i] >= 0  && pstViInfo->stPipeInfo.aPipe[i] < VI_MAX_PIPE_NUM)
        {
            ...
            if (bNeedChn)
            {
                    ...
                    || VI_PARALLEL_VPSS_OFFLINE == enMastPipeMode)
                {
                    ...
                    if (s32Ret != HI_SUCCESS)
                    {
                        LOGE("HI_MPI_VI_DisableChn failed with %#x!\n", s32Ret);
                        return HI_FAILURE;
                    }
                }
            }
...
static HI_S32 LOTO_COMM_VI_CreateSingleVi(SAMPLE_VI_INFO_S* pstViInfo)
{
    HI_S32 s32Ret = HI_SUCCESS;

    s32Ret = LOTO_COMM_VI_StartDev(pstViInfo);

    if (s32Ret != HI_SUCCESS)
    {
        LOGE("LOTO_COMM_VI_StartDev failed !\n");
        return HI_FAILURE;
    }

    //we should bind pipe,then creat pipe
    s32Ret = LOTO_COMM_VI_BindPipeDev(pstViInfo);

    if (s32Ret != HI_SUCCESS)
    {
        LOGE("LOTO_COMM_VI_BindPipeDev failed !\n");
        goto EXIT1;
    }

    s32Ret = LOTO_COMM_VI_StartViPipe(pstViInfo);

    if (s32Ret != HI_SUCCESS)
    {
        LOGE("LOTO_COMM_VI_StartViPipe failed !\n");
        goto EXIT1;
    }

    s32Ret = LOTO_COMM_VI_StartViChn(pstViInfo);

    if (s32Ret != HI_SUCCESS)
    {
        LOGE("LOTO_COMM_VI_StartViChn failed !\n");
        goto EXIT2;
    }
...
EXIT2:
    LOTO_COMM_VI_StopViPipe(pstViInfo);

EXIT1:
    LOTO_COMM_VI_StopDev(pstViInfo);
...
static HI_S32 LOTO_COMM_ModeSwitch_VI_CreateSingleVi(SAMPLE_VI_INFO_S* pstViInfo)
{
    HI_S32 s32Ret = HI_SUCCESS;
    ...
    s32Ret = LOTO_COMM_VI_ModeSwitch_StartViPipe(pstViInfo);
    if (s32Ret != HI_SUCCESS)
    {
        LOGE("LOTO_COMM_VI_StartViPipe failed !\n");
        goto EXIT1;
    ...
    s32Ret = LOTO_COMM_VI_ModeSwitch_StartViChn(pstViInfo);
    if (s32Ret != HI_SUCCESS)
    {
        LOGE("LOTO_COMM_VI_StartViChn failed !\n");
        goto EXIT2;
...
static HI_S32 LOTO_COMM_VI_StartPipe_Chn(SAMPLE_VI_INFO_S* pstViInfo)
{
    HI_S32 s32Ret = HI_SUCCESS;


    s32Ret = LOTO_COMM_VI_ModeSwitch_EnableViPipe(pstViInfo);
...
static HI_S32 LOTO_COMM_VI_DestroySingleVi(SAMPLE_VI_INFO_S* pstViInfo)
{
    LOTO_COMM_VI_StopViChn(pstViInfo);
...
static HI_S32 LOTO_COMM_VI_DestroySinglePipe_Chn(SAMPLE_VI_INFO_S* pstViInfo)
{
   LOTO_COMM_VI_StopViChn(pstViInfo);
...
HI_S32 LOTO_COMM_VI_CreateVi(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    HI_S32              i, j;
    HI_S32              s32ViNum;
    HI_S32              s32Ret = HI_SUCCESS;
    ...
    for (i = 0; i < pstViConfig->s32WorkingViNum; i++)
    {
        ...
        s32Ret = LOTO_COMM_VI_CreateSingleVi(pstViInfo);

        if (s32Ret != HI_SUCCESS)
        {
            LOGE("LOTO_COMM_VI_CreateSingleVi failed !\n");
            goto EXIT;
        }
    }
...
EXIT:
    ...
    for (j = 0; j < i; j++)
    {
        ...
        LOTO_COMM_VI_DestroySingleVi(pstViInfo);
    }
...
HI_S32 LOTO_COMM_ModeSwitch_VI_CreateVi(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    HI_S32              i, j;
    HI_S32              s32ViNum;
    HI_S32              s32Ret = HI_SUCCESS;
    ...
    for (i = 0; i < pstViConfig->s32WorkingViNum; i++)
    {
        ...
        s32Ret = LOTO_COMM_ModeSwitch_VI_CreateSingleVi(pstViInfo);
...
HI_S32 LOTO_COMM_ModeSwitch_VI_StartPipe_Chn(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    HI_S32              i, j;
    HI_S32              s32ViNum;
    HI_S32              s32Ret = HI_SUCCESS;
    ...
    for (i = 0; i < pstViConfig->s32WorkingViNum; i++)
    {
        ...
        s32Ret = LOTO_COMM_VI_StartPipe_Chn(pstViInfo);
...
EXIT:
    ...
    for (j = 0; j < i; j++)
    {
        ...
        LOTO_COMM_VI_DestroySinglePipe_Chn(pstViInfo);
    }

    return s32Ret;
}

HI_S32 LOTO_COMM_VI_DestroyVi(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    HI_S32            i;
    HI_S32            s32ViNum;
    SAMPLE_VI_INFO_S* pstViInfo = HI_NULL;
...
HI_S32 LOTO_COMM_VI_StartIsp(SAMPLE_VI_INFO_S* pstViInfo)
{
    HI_S32              i;
    HI_BOOL             bNeedPipe;
    HI_S32              s32Ret = HI_SUCCESS;
    ...
    for (i = 0; i < WDR_MAX_PIPE_NUM; i++)
    {
        if (pstViInfo->stPipeInfo.aPipe[i] >= 0  && pstViInfo->stPipeInfo.aPipe[i] < VI_MAX_PIPE_NUM)
        {
            ...
            LOTO_COMM_ISP_GetIspAttrBySns(pstViInfo->stSnsInfo.enSnsType, &stPubAttr);

            stPubAttr.enWDRMode = pstViInfo->stDevInfo.enWDRMode;

            LOGD("stPubAttr.enWDRMode=%d, ViPipe=%d, u32SnsId=%d\n",stPubAttr.enWDRMode, ViPipe, u32SnsId);
            ...
            s32Ret = LOTO_COMM_ISP_Sensor_Regiter_callback(ViPipe, u32SnsId);

            if (HI_SUCCESS != s32Ret)
            {
                LOGE("register sensor %d to ISP %d failed\n", u32SnsId, ViPipe);
                LOTO_COMM_ISP_Stop(ViPipe);
                return HI_FAILURE;
            }
            ...
            {
                s32Ret = LOTO_COMM_ISP_BindSns(ViPipe, u32SnsId, pstViInfo->stSnsInfo.enSnsType, -1);

                if (HI_SUCCESS != s32Ret)
                {
                    LOGE("register sensor %d bus id %d failed\n", u32SnsId, pstViInfo->stSnsInfo.s32BusId);
                    LOTO_COMM_ISP_Stop(ViPipe);
                    return HI_FAILURE;
                }
            }
            ...
            {
                s32Ret = LOTO_COMM_ISP_BindSns(ViPipe, u32SnsId, pstViInfo->stSnsInfo.enSnsType, pstViInfo->stSnsInfo.s32BusId);
            ...
            s32Ret = LOTO_COMM_ISP_Aelib_Callback(ViPipe);

            if (HI_SUCCESS != s32Ret)
            {
                LOGE("LOTO_COMM_ISP_Aelib_Callback failed\n");
                LOTO_COMM_ISP_Stop(ViPipe);
                return HI_FAILURE;
            }

            s32Ret = LOTO_COMM_ISP_Awblib_Callback(ViPipe);

            if (HI_SUCCESS != s32Ret)
            {
                LOGE("LOTO_COMM_ISP_Awblib_Callback failed\n");
                LOTO_COMM_ISP_Stop(ViPipe);
                return HI_FAILURE;
            }
            ...
            if (s32Ret != HI_SUCCESS)
            {
                LOGE("Init Ext memory failed with %#x!\n", s32Ret);
                LOTO_COMM_ISP_Stop(ViPipe);
                return HI_FAILURE;
            }
            ...
            if (s32Ret != HI_SUCCESS)
            {
                LOGE("ISP Init failed with %#x!\n", s32Ret);
                LOTO_COMM_ISP_Stop(ViPipe);
                return HI_FAILURE;
            }

            s32Ret = LOTO_COMM_ISP_Run(ViPipe);

            if (s32Ret != HI_SUCCESS)
            {
                LOGE("ISP Run failed with %#x!\n", s32Ret);
                LOTO_COMM_ISP_Stop(ViPipe);
                return HI_FAILURE;
            }
        }
...
HI_S32 LOTO_COMM_VI_StopIsp(SAMPLE_VI_INFO_S* pstViInfo)
{
    HI_S32  i;
    HI_BOOL bNeedPipe;
    VI_PIPE ViPipe;
...
HI_S32 LOTO_COMM_VI_CreateIsp(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    HI_S32              i;
    HI_S32              s32ViNum;
    HI_S32              s32Ret = HI_SUCCESS;
    ...
    for (i = 0; i < pstViConfig->s32WorkingViNum; i++)
    {
        ...
        s32Ret = LOTO_COMM_VI_StartIsp(pstViInfo);

        if (s32Ret != HI_SUCCESS)
        {
            LOGE("LOTO_COMM_VI_StartIsp failed !\n");
            return HI_FAILURE;
        }
...
HI_S32 LOTO_COMM_VI_DestroyIsp(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    HI_S32              i;
    HI_S32              s32ViNum;
    HI_S32              s32Ret = HI_SUCCESS;
    ...
    for (i = 0; i < pstViConfig->s32WorkingViNum; i++)
    {
        ...
        s32Ret = LOTO_COMM_VI_StopIsp(pstViInfo);

        if (s32Ret != HI_SUCCESS)
        {
            LOGE("LOTO_COMM_VI_StopIsp failed !\n");
            return HI_FAILURE;
        }
    }
...
HI_S32 LOTO_COMM_VI_StartVi(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    HI_S32 s32Ret = HI_SUCCESS;
    ...
    s32Ret = LOTO_COMM_VI_StartMIPI(pstViConfig);
    if (s32Ret != HI_SUCCESS)
    {
        LOGE("LOTO_COMM_VI_StartMIPI failed!\n");
        return HI_FAILURE;
    }
    ...
    if (s32Ret != HI_SUCCESS)
    {
        LOGE("LOTO_COMM_VI_SetParam failed!\n");
        return HI_FAILURE;
    }

    s32Ret = LOTO_COMM_VI_CreateVi(pstViConfig);
    if (s32Ret != HI_SUCCESS)
    {
        LOGE("LOTO_COMM_VI_CreateVi failed!\n");
        return HI_FAILURE;
    }
#if 1
    s32Ret = LOTO_COMM_VI_CreateIsp(pstViConfig);
    if (s32Ret != HI_SUCCESS)
    {
        LOTO_COMM_VI_DestroyVi(pstViConfig);
        LOGE("LOTO_COMM_VI_CreateIsp failed!\n");
        return HI_FAILURE;
    }
#endif
    return s32Ret;
...
HI_S32 LOTO_COMM_VI_StopVi(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    HI_S32 s32Ret = HI_SUCCESS;

        s32Ret = LOTO_COMM_VI_DestroyIsp(pstViConfig);

        if (s32Ret != HI_SUCCESS)
        {
            LOGE("LOTO_COMM_VI_DestroyIsp failed !\n");
            return HI_FAILURE;
        }
    ...
    s32Ret = LOTO_COMM_VI_DestroyVi(pstViConfig);

    if (s32Ret != HI_SUCCESS)
    {
        LOGE("LOTO_COMM_VI_DestroyVi failed !\n");
        return HI_FAILURE;
    }

    s32Ret = LOTO_COMM_VI_StopMIPI(pstViConfig);

    if (s32Ret != HI_SUCCESS)
    {
        LOGE("LOTO_COMM_VI_StopMIPI failed !\n");
        return HI_FAILURE;
    }
...
HI_S32 LOTO_COMM_VI_SwitchISPMode(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    HI_S32              i;
    HI_S32              s32ViNum;
    HI_S32              s32Ret = HI_SUCCESS;
    ...
    for (i = 0; i < pstViConfig->s32WorkingViNum; i++)
    {
        ...
        for (i = 0; i < WDR_MAX_PIPE_NUM; i++)
        {
            if ( pstViInfo->stPipeInfo.aPipe[i] >= 0 && pstViInfo->stPipeInfo.aPipe[i] < VI_MAX_PIPE_NUM)
            {
                ...
                LOGE("LOTO_COMM_VI_CreateIsp enWDRMode is %d!\n", stPubAttr.enWDRMode);
    ...
    LOTO_COMM_VI_StartPipe_Chn(pstViInfo);

    return HI_SUCCESS;
}


HI_S32  LOTO_COMM_VI_SwitchMode_StopVI(SAMPLE_VI_CONFIG_S* pstViConfigSrc)
{

    HI_S32              s32Ret = HI_SUCCESS;

    s32Ret = LOTO_COMM_VI_DestroyVi(pstViConfigSrc);
    ...
    s32Ret = LOTO_COMM_VI_StopMIPI(pstViConfigSrc);
...
HI_S32  LOTO_COMM_VI_SwitchMode(SAMPLE_VI_CONFIG_S* pstViConfigDes)
{

    HI_S32 s32Ret = HI_SUCCESS;

    s32Ret = LOTO_COMM_VI_StartMIPI(pstViConfigDes);
    if (s32Ret != HI_SUCCESS)
    {
        LOGE("LOTO_COMM_VI_StartMIPI failed!\n");
        return HI_FAILURE;
    ...
    s32Ret =  LOTO_COMM_ModeSwitch_VI_CreateVi(pstViConfigDes);
    if (s32Ret != HI_SUCCESS)
    {
        LOGE("LOTO_COMM_VI_CreateVi failed!\n");
        return HI_FAILURE;
    ...
    s32Ret = LOTO_COMM_VI_SwitchISPMode(pstViConfigDes);
    if (s32Ret != HI_SUCCESS)
    {
        LOGE("LOTO_COMM_ModeSwitch_VI_CreateIsp!\n");
        return HI_FAILURE;
    }
...
int LOTO_COMM_VI_ExitMpp( int s32poolId)
{
    if (s32poolId < 0)
    {
        if (HI_MPI_SYS_Exit())
...
******************************************************************************/
HI_S32 LOTO_COMM_VI_GetWDRModeBySensor(SAMPLE_SNS_TYPE_E enMode, WDR_MODE_E* penWDRMode)
{
    HI_S32 s32Ret = HI_SUCCESS;
...
******************************************************************************/
HI_S32 LOTO_COMM_VI_GetPipeBySensor(SAMPLE_SNS_TYPE_E enMode, SAMPLE_PIPE_INFO_S* pstPipeInfo)
{
    HI_S32 s32Ret = HI_SUCCESS;
...
******************************************************************************/
HI_S32 LOTO_COMM_VI_GetSizeBySensor(SAMPLE_SNS_TYPE_E enMode, PIC_SIZE_E* penSize)
{
    HI_S32 s32Ret = HI_SUCCESS;
    ...
    switch (enMode)
    {
        ...
        // case SONY_IMX274_MIPI_2M_30FPS_12BIT:
        // case SONY_IMX274_MIPI_2M_30FPS_12BIT_WDR2TO1:
        //     *penSize = PIC_1080P;
        //     break;
...
/******************************************************************************
* funciton : Get Framerate by diffrent sensor
******************************************************************************/
HI_S32 LOTO_COMM_VI_GetFrameRateBySensor(SAMPLE_SNS_TYPE_E enMode, HI_U32* pu32FrameRate)
{
    HI_S32 s32Ret = HI_SUCCESS;
...
HI_VOID LOTO_COMM_VI_GetSensorInfo(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    HI_S32 i;
...
combo_dev_t LOTO_COMM_VI_GetComboDevBySensor(SAMPLE_SNS_TYPE_E enMode, HI_S32 s32SnsIdx)
{
    combo_dev_t dev = 0;
...
HI_S32 LOTO_COMM_VI_ConvertBitPixel(HI_U8 *pu8Data, HI_U32 u32DataNum, HI_U32 u32BitWidth, HI_U16 *pu16OutData)
{
...
static HI_S32 LOTO_COMM_VI_BitWidth2PixelFormat(HI_U32 u32Nbit, PIXEL_FORMAT_E *penPixelFormat)
{
...
HI_S32 LOTO_COMM_VI_SaveUncompressRaw(VIDEO_FRAME_S* pVBuf, HI_U32 u32Nbit, FILE* pfd)
{
    HI_U32  u32Height;
    HI_U64  u64PhyAddr;
    HI_U64  u64Size;
    ...
    LOTO_COMM_VI_BitWidth2PixelFormat(u32Nbit, &enPixelFormat);
    if (enPixelFormat != pVBuf->enPixelFormat)
    {
        LOGE("invalid pixel format:%d, u32Nbit: %d\n", pVBuf->enPixelFormat, u32Nbit);
    ...
    /* save Y ----------------------------------------------------------------*/
    LOGE("saving......dump data......u32Stride[0]: %d, width: %d\n", pVBuf->u32Stride[0], pVBuf->u32Width);
    ...
    for (u32Height = 0; u32Height < pVBuf->u32Height; u32Height++)
    {
        ...
        {
            LOTO_COMM_VI_ConvertBitPixel(pu8Data, pVBuf->u32Width, u32Nbit, pu16Data);
...
HI_S32 LOTO_COMM_VI_SaveCompressedRaw(VIDEO_FRAME_S* pVBuf, HI_U32 u32Nbit, FILE* pfd)
{
    HI_U32  u32Height;
    HI_U64  u64PhyAddr;
    HI_U64  u64Size;
...
HI_S32 LOTO_COMM_VI_SaveRaw(VIDEO_FRAME_S* pVBuf, HI_U32 u32Nbit, FILE* pfd)
{
    if(COMPRESS_MODE_NONE == pVBuf->enCompressMode)
    {
        return LOTO_COMM_VI_SaveUncompressRaw(pVBuf, u32Nbit, pfd);
    }
    else
    {
        return LOTO_COMM_VI_SaveCompressedRaw(pVBuf, u32Nbit, pfd);
    }

}
HI_U32 LOTO_COMM_VI_PixelFormat2BitWidth(PIXEL_FORMAT_E  enPixelFormat)
{
...
char* LOTO_COMM_VI_CompressMode2String(COMPRESS_MODE_E enCompressMode)
{
...
int LOTO_COMM_VI_SaveCompressParam(VI_CMP_PARAM_S* pCmpParam, FILE* pfd)
{
...
HI_VOID* LOTO_COMM_VI_DumpRaw(HI_VOID* arg)
{
    HI_S32      s32Ret;
    VI_PIPE     ViPipe;
    HI_S32      s32Cnt;
    ...
    if (s32Cnt < 1)
    {
        LOGE("You really want to dump %d frame? That is impossible !\n", s32Cnt);
        return NULL;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        LOGE("HI_MPI_VI_GetPipeFrame failed with %#x!\n", s32Ret);
        return HI_NULL;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        LOGE("HI_MPI_VI_ReleasePipeFrame failed with %#x!\n", s32Ret);
        return HI_NULL;
    }

    u32Width    = stVideoFrame.stVFrame.u32Width;
    u32Height   = stVideoFrame.stVFrame.u32Height;
    u32BitWidth = LOTO_COMM_VI_PixelFormat2BitWidth(stVideoFrame.stVFrame.enPixelFormat);
    ...
    snprintf(name, sizeof(name), VI_DATA_PATH"/%s_pipe%d_w%d_h%d_%dbits_%s_%d.raw",
             ...
             LOTO_COMM_VI_CompressMode2String(stVideoFrame.stVFrame.enCompressMode), s32Cnt);
    pfile = fopen(name, "ab");
    ...
    if(COMPRESS_MODE_NONE != stVideoFrame.stVFrame.enCompressMode)
    {
        ...
        {
            LOGE("HI_MPI_VI_GetPipeCmpParam failed with %#x!\n", s32Ret);
            goto end;
        }

        LOTO_COMM_VI_SaveCompressParam(&stCmpPara, pfile);
    }
    ...
    while ((HI_TRUE == g_stViDumpRawThreadInfo.bDump) && (s32DumpCnt < s32Cnt))
    {
        ...
        s32Ret = LOTO_COMM_VI_SaveRaw(&stVideoFrame.stVFrame, u32BitWidth, pfile);

        if (HI_SUCCESS != s32Ret)
        {
            LOGE("LOTO_COMM_VI_SaveRaw failed with %#x!\n", s32Ret);
            goto end;
        }
...
HI_S32 LOTO_COMM_VI_StartDumpRawThread(VI_PIPE ViPipe, HI_S32 s32Cnt, const HI_CHAR* pzsName)
{
    HI_S32 s32Ret;
    ...
    s32Ret = pthread_create(&g_stViDumpRawThreadInfo.ThreadId, pattr, LOTO_COMM_VI_DumpRaw, (HI_VOID*)&g_stViDumpRawThreadInfo);

    if (0 != s32Ret)
    {
        LOGE("create GetViFrame thread failed! %s\n", strerror(s32Ret));
        goto out;
    }
...
HI_S32 LOTO_COMM_VI_StopDumpRawThread(HI_VOID)
{
...
HI_S32 SAMPLE_VI_GetFrameBlkInfo(SAMPLE_VI_FRAME_CONFIG_S *pstFrmCfg, HI_S32 s32FrmCnt, SAMPLE_VI_FRAME_INFO_S *pastViFrameInfo)
{
    ...
    {
        LOGE("HI_MPI_VB_CreatePool failed!\n");
        return HI_FAILURE;
    }
    ...
    for (i = 0; i < s32FrmCnt; i++)
    {
        ...
        {
            LOGE("HI_MPI_VB_GetBlock err! size:%d\n", u32Size);
            return HI_FAILURE;
        }
        ...
        if (0 == u64PhyAddr)
        {
            LOGE("HI_MPI_VB_Handle2PhysAddr err!\n");
            return HI_FAILURE;
        }
...
HI_S32 SAMPLE_VI_COMM_ReleaseFrameBlkInfo(HI_S32 s32FrmCnt, SAMPLE_VI_FRAME_INFO_S *pastViFrameInfo)
{
    ...
    for (i = 0; i < s32FrmCnt; i++)
    {
        ...
        if (s32Ret != HI_SUCCESS)
        {
            LOGE("HI_MPI_VB_ReleaseBlock block 0x%x failure\n", VbBlk);
        }
        ...
        if (s32Ret != HI_SUCCESS)
        {
            LOGE("HI_MPI_SYS_Munmap failure!\n");
        }
    }
...
HI_VOID LOTO_COMM_VI_SaveFpnFile(ISP_FPN_TYPE_E enFpnType, ISP_FPN_FRAME_INFO_S *pVBuf, FILE *pfd)
{
    HI_U8   *pU8VBufVirt_Y;
    HI_U32  u32FpnHeight;
    HI_S32  i;
...
HI_VOID LOTO_COMM_VI_ReadFpnFile(ISP_FPN_FRAME_INFO_S *pstFpnFrmInfo, FILE *pfd)
{
...
HI_S32 LOTO_COMM_VI_FpnCalibrateConfig(VI_PIPE ViPipe, SAMPLE_VI_FPN_CALIBRATE_INFO_S *pstViFpnCalibrateInfo)
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
        LOGE("SAMPLE_VI_GetFrameBlkInfo failed!\n");
        return s32Ret;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        LOGE("disable vi chn %d failed!", ViChn);
        goto EXIT0;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        LOGE("HI_MPI_ISP_FPNCalibrate err: 0x%x\n", s32Ret);
        goto EXIT1;
    }
    ...
    LOTO_COMM_VI_SaveFpnFile(stCalibrateAttr.enFpnType, &stCalibrateAttr.stFpnCaliFrame, pfd);
...
EXIT1:
    ...
    if (HI_SUCCESS != s32Ret)
    {
        LOGE("enable vi chn %d failed!", ViChn);
    }
EXIT0:
...
HI_S32 LOTO_COMM_VI_FpnCorrectionConfig(VI_PIPE ViPipe, SAMPLE_VI_FPN_CORRECTION_INFO_S *pstViFpnCorrectionInfo)
{
    SAMPLE_VI_FRAME_CONFIG_S stFrmCfg;
    ...
    if (HI_SUCCESS != s32Ret)
    {
        LOGE("VI_MST_COMM_GetFrameBlkInfo failed!\n");
        return s32Ret;
    }
    ...
    if (NULL == pfd)
    {
        LOGE("open file %s err!\n", FpnFileName);
        goto EXIT0;
    }

    LOTO_COMM_VI_ReadFpnFile(&stFPNAttr.stFpnFrmInfo, pfd);
...
HI_S32 LOTO_COMM_VI_DisableFpnCorrection(VI_PIPE ViPipe, SAMPLE_VI_FPN_CORRECTION_INFO_S *pstViFpnCorrectionInfo)
{
    ISP_FPN_ATTR_S stFPNAttr;
    HI_S32 s32Ret;
    ...
    if (HI_SUCCESS != s32Ret)
    {
        LOGE("get fpn correction fail, ret 0x%x\n", s32Ret);
        return HI_FAILURE;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        LOGE("set fpn correction fail, ret 0x%x\n", s32Ret);
        return HI_FAILURE;
    }
...
HI_S32 LOTO_COMM_VI_Load_UserPic(const char *pszYuvFile, VI_USERPIC_ATTR_S *pstUsrPic, SAMPLE_VI_FRAME_INFO_S *pstViFrameInfo)
{
    FILE *pfd;
    SAMPLE_VI_FRAME_CONFIG_S stFrmCfg;
    HI_S32 s32Ret;
...
HI_VOID LOTO_COMM_VI_Release_UserPic(SAMPLE_VI_FRAME_INFO_S *pstViFrameInfo)
{
    SAMPLE_VI_COMM_ReleaseFrameBlkInfo(1, pstViFrameInfo);
}
