...
//         {
//             LOGE("HI_MPI_SYS_SetMemConfig with %#x!\n", s32Ret);
//             return HI_FAILURE;
//         }
//     }
//     return HI_SUCCESS;
...
******************************************************************************/
HI_S32 LOTO_COMM_VENC_GetFilePostfix(PAYLOAD_TYPE_E enPayload, char* szFilePostfix)
{
...
/**
 * @brief Set GOP attribute
 * 
 * @param enGopMode GOP Mode
 * @param pstGopAttr Mode attribute pointer
...
HI_S32 LOTO_COMM_VENC_GetGopAttr(VENC_GOP_MODE_E enGopMode,VENC_GOP_ATTR_S *pstGopAttr)
{
    switch(enGopMode)
    {
        ...
        case VENC_GOPMODE_SMARTP:
            ...
            pstGopAttr->stSmartP.s32BgQpDelta  = 4;     // 长期参考帧和 P 帧的 QP 差值
            pstGopAttr->stSmartP.s32ViQpDelta  = 2;     // 虚拟 I 帧相对于普通 P 帧的 QP 差值
            // pstGopAttr->stSmartP.u32BgInterval = 90;    // 长期参考帧的间隔
            pstGopAttr->stSmartP.u32BgInterval = 60;
        break;

        case VENC_GOPMODE_DUALP:
            pstGopAttr->enGopMode  = VENC_GOPMODE_DUALP;
            pstGopAttr->stDualP.s32IPQpDelta  = 4;      // I 帧相对普通 P 帧的QP差值
            pstGopAttr->stDualP.s32SPQpDelta  = 2;      // Special p 帧相对普通 P 帧的 QP 差值
            pstGopAttr->stDualP.u32SPInterval = 3;      // Special P 帧的间隔
            // pstGopAttr->stDualP.u32SPInterval = 6;
        break;
...
HI_S32 LOTO_COMM_VENC_Getdcfinfo(char* SrcJpgPath, char* DstThmPath)
{
...
******************************************************************************/
HI_S32 LOTO_COMM_VENC_SaveStream(FILE* pFd, VENC_STREAM_S* pstStream)
{
    HI_S32 i;
...
******************************************************************************/
HI_S32 LOTO_COMM_VENC_SaveStream_PhyAddr(FILE* pFd, VENC_STREAM_BUF_INFO_S *pstStreamBuf, VENC_STREAM_S* pstStream)
{
    HI_U32 i,j;
...
HI_S32 LOTO_COMM_VENC_CloseReEncode(VENC_CHN VencChn)
{
    HI_S32 s32Ret;
...
HI_S32 LOTO_COMM_VENC_Creat(VENC_CHN VencChn, PAYLOAD_TYPE_E enType,  PIC_SIZE_E enSize, SAMPLE_RC_E enRcMode, HI_U32  u32Profile, HI_BOOL bRcnRefShareBuf,VENC_GOP_ATTR_S *pstGopAttr)
{
    ...
    HI_U32                 u32FrameRate;
    HI_U32                 u32StatTime;         // Bitrate statistics time, Units: Seconds(s)
    HI_U32                 u32Gop = 30;         
    // HI_U32                 u32Gop = 12;         
     
    s32Ret = LOTO_COMM_SYS_GetPicSize( enSize, &stPicSize);
    if (HI_SUCCESS != s32Ret)
    {
    ...
    LOTO_COMM_VI_GetSensorInfo(&stViConfig);
    if(SAMPLE_SNS_TYPE_BUTT == stViConfig.astViInfo[0].stSnsInfo.enSnsType)
    {
        LOGE("Not set SENSOR%d_TYPE !\n",0);
        return HI_FALSE;
    }
    s32Ret = LOTO_COMM_VI_GetFrameRateBySensor(stViConfig.astViInfo[0].stSnsInfo.enSnsType, &u32FrameRate);
    if (HI_SUCCESS != s32Ret)
    {
        LOGE("LOTO_COMM_VI_GetFrameRateBySensor failed!\n");
        return s32Ret;
    }
    ...
    ******************************************/
    ...
    stVencChnAttr.stVencAttr.u32MaxPicWidth  = stPicSize.u32Height;
    stVencChnAttr.stVencAttr.u32MaxPicHeight = stPicSize.u32Width;
    stVencChnAttr.stVencAttr.u32PicWidth     = stPicSize.u32Height;                             /* the picture width */
    stVencChnAttr.stVencAttr.u32PicHeight    = stPicSize.u32Width;                              /* the picture height */
#endif

#ifdef VPSS_CROP
    stVencChnAttr.stVencAttr.u32MaxPicWidth  = 1080;
    stVencChnAttr.stVencAttr.u32MaxPicHeight = 1080;
    stVencChnAttr.stVencAttr.u32PicWidth     = 1080;                             /* the picture width */
    stVencChnAttr.stVencAttr.u32PicHeight    = 1080;                              /* the picture height */

#ifdef SNS_IMX335
    stVencChnAttr.stVencAttr.u32MaxPicWidth  = 1944;
    stVencChnAttr.stVencAttr.u32MaxPicHeight = 1944;
    stVencChnAttr.stVencAttr.u32PicWidth     = 1944;                             /* the picture width */
    stVencChnAttr.stVencAttr.u32PicHeight    = 1944;                              /* the picture height */

    // stVencChnAttr.stVencAttr.u32MaxPicWidth  = 1080;
    // stVencChnAttr.stVencAttr.u32MaxPicHeight = 1080;
    // stVencChnAttr.stVencAttr.u32PicWidth     = 1080;
    ...
    stVencChnAttr.stVencAttr.u32BufSize      = stVencChnAttr.stVencAttr.u32PicWidth * stVencChnAttr.stVencAttr.u32PicHeight * 2;    /* stream buffer size */
    ...
    switch (enType)
    {
        case PT_H265:
        {
            ...
            {
                LOGE("%s,%d,enRcMode(%d) not support\n",__FUNCTION__,__LINE__,enRcMode);
                return HI_FAILURE;
            }
        ...
        case PT_H264:
        {
            if (SAMPLE_RC_CBR == enRcMode)
            {
                ...
                {
                    case PIC_720P:
                        stH264Cbr.u32BitRate         = 1024 * 2;        //1024 * 3 + 1024*u32FrameRate/30;
                        break;
                    case PIC_1080P:
                        // stH264Cbr.u32BitRate         = 1024 * 2;        //1024 * 2 + 2048*u32FrameRate/30;
                        stH264Cbr.u32BitRate         = 1024 * 3;
                        // stH264Cbr.u32BitRate         = 1024 * 4;
                        break;
                    case PIC_2592x1944:
                        // stH264Cbr.u32BitRate         = 1024 * 6;        // + 3072*u32FrameRate/30;
                        stH264Cbr.u32BitRate         = 1024 * 5;        // + 3072*u32FrameRate/30;
                ...
                LOGD("H264_CBR: u32BitRate = %u kbps\n", stH264Cbr.u32BitRate);
            ...
            else if (SAMPLE_RC_VBR == enRcMode)
            {
                ...
                switch (enSize)
                {
                    case PIC_360P:
                        ...
                        break;
                    case PIC_720P:
                        // stH264Vbr.u32MaxBitRate = 1024 * 2   + 1024*u32FrameRate/30; /* 30fps --> 3M */
                        stH264Vbr.u32MaxBitRate = 1024 * 2;
                        // stH264Vbr.u32MaxBitRate = 1024 * 3;
                        // stH264Vbr.u32MaxBitRate = 1024 * 4;
                        break;
                    case PIC_1080P:
                        // stH264Vbr.u32MaxBitRate = 1024 * 2   + 2048*u32FrameRate/30; /* 30fps --> 4M */
                        // stH264Vbr.u32MaxBitRate = 1024 * 3;
                        stH264Vbr.u32MaxBitRate = 1024 * 4;
                        break;
                    case PIC_2592x1944:
                ...
                LOGD("H264_VBR: u32MaxBitrate = %u kbps\n", stH264Vbr.u32MaxBitRate);
        ...
        case PT_MJPEG:
        {
                     ...
                     (SAMPLE_RC_QVBR == enRcMode)||(SAMPLE_RC_CVBR == enRcMode))
            {
                ...
                {
                    LOGE("Mjpege not support AVBR, so change rcmode to VBR!\n");
                }
            ...
            {
                LOGE("cann't support other mode(%d) in this version!\n",enRcMode);
                return HI_FAILURE;
            }
        }
        break;
        case PT_JPEG:
            ...
            break;
        default:
            LOGE("cann't support this enType (%d) in this version!\n",enType);
    ...
    else
    {
        ...
        if((VENC_GOPMODE_BIPREDB == pstGopAttr->enGopMode)&&(PT_H264 == enType))
        {
            if(0 == stVencChnAttr.stVencAttr.u32Profile)
            {
                ...
                LOGE("H.264 base profile not support BIPREDB, so change profile to main profile!\n");
            }
        }
        ...
        if((VENC_RC_MODE_H264QPMAP == stVencChnAttr.stRcAttr.enRcMode)||(VENC_RC_MODE_H265QPMAP == stVencChnAttr.stRcAttr.enRcMode))
        {
            if(VENC_GOPMODE_ADVSMARTP == pstGopAttr->enGopMode)
            {
                ...
                LOGE("advsmartp not support QPMAP, so change gopmode to smartp!\n");
            }
        }
    }
    ...
    s32Ret = LOTO_COMM_VENC_CloseReEncode(VencChn);
    if (HI_SUCCESS != s32Ret)
    {
...
HI_S32 LOTO_COMM_VENC_Start(VENC_CHN VencChn, PAYLOAD_TYPE_E enType,  PIC_SIZE_E enSize, SAMPLE_RC_E enRcMode, HI_U32  u32Profile, HI_BOOL bRcnRefShareBuf, VENC_GOP_ATTR_S *pstGopAttr)
{
    ...
    ******************************************/
    s32Ret = LOTO_COMM_VENC_Creat(VencChn, enType, enSize, enRcMode, u32Profile, bRcnRefShareBuf, pstGopAttr);
    if (HI_SUCCESS != s32Ret)
    {
        LOGE("LOTO_COMM_VENC_Creat faild with%#x! \n", s32Ret);
        return HI_FAILURE;
    }
    /******************************************
...
HI_S32 LOTO_COMM_VENC_Stop(VENC_CHN VencChn)
{
    ...
    ******************************************/
    ...
    if (HI_SUCCESS != s32Ret)
    {
        LOGE("HI_MPI_VENC_StopRecvPic vechn[%d] failed with %#x!\n", \
                   VencChn, s32Ret);
        return HI_FAILURE;
    }    /******************************************
...
//                     s32Ret = LOTO_COMM_VENC_SaveStream(pFile, &stStream);
//                     if (HI_SUCCESS != s32Ret)
//                     {
...
//                     s32Ret = LOTO_COMM_VENC_SaveStream_PhyAddr(pFile, &stStreamBufInfo, &stStream);
//                     if (HI_SUCCESS != s32Ret)
//                     {
//                         LOGE("save snap picture failed!\n");
...
//                         s32Ret = LOTO_COMM_VENC_Getdcfinfo(acFile, acFile_dcf);
//                         if (HI_SUCCESS != s32Ret)
//                         {
//                             LOGE("save thm picture failed!\n");
...
HI_VOID* LOTO_COMM_QpmapSendFrameProc(HI_VOID* p)
{
    HI_U32 i,j,VeChnCnt;
    HI_S32 s32Ret;
    ...
    if(HI_SUCCESS != s32Ret)
    {
        LOGE("HI_MPI_SYS_MmzAlloc err:0x%x",s32Ret);
        return NULL;
    }
    ...
    if(HI_SUCCESS != s32Ret)
    {
        LOGE("HI_MPI_VPSS_GetChnAttr err:0x%x",s32Ret);
    ...
    if(HI_SUCCESS != s32Ret)
    {
        LOGE("HI_MPI_VPSS_SetChnAttr err:0x%x",s32Ret);
    ...
    while(HI_TRUE == pstPara->bThreadStart)
    {
        ...
        if(HI_SUCCESS != s32Ret)
        {
            LOGE("HI_MPI_VPSS_GetChnFrame err:0x%x\n",s32Ret);
            continue;
        }
        ...
        for(VeChnCnt=0; VeChnCnt<pstPara->s32Cnt; VeChnCnt++)
        {
            ...
            if(HI_SUCCESS != s32Ret)
            {
                LOGE("HI_MPI_VENC_SendFrame err:0x%x\n",s32Ret);
                break;
            }
        }
        if(HI_SUCCESS != s32Ret)
        {
            ...
            if(HI_SUCCESS != s32Ret)
            {
                LOGE("HI_MPI_VPSS_ReleaseChnFrame err:0x%x",s32Ret);
                goto err_out;
            }
            continue;
        }
...
err_out:
    ...
    if(HI_SUCCESS != s32Ret)
    {
        LOGE("HI_MPI_SYS_MmzFree err:0x%x",s32Ret);
        return NULL;
    }
...
//     return pthread_create(&gs_VencQpmapPid, 0, LOTO_COMM_QpmapSendFrameProc, (HI_VOID*)&stQpMapSendFramePara);
// }
            ...
            for (i = 0; i < s32ChnTotal; i++)
            {
                if (FD_ISSET(VencFd[i], &read_fds))
                {
                    ...
                    *******************************************************/
                    ...
                    if (HI_SUCCESS != s32Ret)
                    {
                        LOGE("HI_MPI_VENC_QueryStatus chn[%d] failed with %#x!\n", i, s32Ret);
                        break;
                    }
                    ...
                    *******************************************************/
                    ...
                    if (HI_SUCCESS != s32Ret)
                    {
                        LOGE("HI_MPI_VENC_ReleaseStream failed!\n");
                        free(stStream.pstPack);
                        stStream.pstPack = NULL;
                        break;
                    }
...
******************************************************************************/
HI_VOID* LOTO_COMM_VENC_GetVencStreamProc_Svc_t(void* p)
{
    HI_S32 i = 0;
    HI_S32 s32Cnt = 0;
    HI_S32 s32ChnTotal;
                    ...
                    for (s32Cnt = 0; s32Cnt < 3; s32Cnt++)
                    {
                        switch (s32Cnt)
                        {
                            case 0:
                                ...
                                {
                                    #ifndef __HuaweiLite__
                                    s32Ret = LOTO_COMM_VENC_SaveStream(pFile[i+s32Cnt], &stStream);
                                    #else
                                    s32Ret = LOTO_COMM_VENC_SaveStream_PhyAddr(pFile[i + s32Cnt], &stStreamBufInfo[i], &stStream);
                                    #endif
                                }
                                break;
                            case 1:
...
//     gs_stPara.bThreadStart = HI_TRUE;
//     gs_stPara.s32Cnt = s32Cnt;
//     return pthread_create(&gs_VencPid, 0, LOTO_COMM_VENC_GetVencStreamProc_Svc_t, (HI_VOID*)&gs_stPara);
// }
...
HI_S32 LOTO_COMM_VENC_PlanToSemi(HI_U8* pY, HI_S32 yStride,
