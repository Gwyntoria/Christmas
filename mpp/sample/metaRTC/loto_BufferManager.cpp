...
extern YangAudioEncoderBuffer *p_audioBuffer;
extern YangVideoEncoderBuffer *p_videoBuffer;
...
HI_S32 HisiPutH264DataToBuffer(VENC_STREAM_S *pstStream)
{
    ...
    for (i = 0; i < pstStream->u32PackCount; i++)
    {
        ...
        memcpy(videoFrame.payload + off, pstStream->pstPack[i].pu8Addr + pstStream->pstPack[i].u32Offset, pstStream->pstPack[i].u32Len - pstStream->pstPack[i].u32Offset);
    ...
    p_videoBuffer->putEVideo(&videoFrame);
...
HI_S32 HisiPutAACDataToBuffer(AUDIO_STREAM_S *aacStream)
{
    ...
    p_audioBuffer->putAudio(&audioFrame);
...
HI_S32 HisiPutOpusDataToBuffer(HI_U8 *opus_data, HI_U32 data_size, HI_U64 timestamp)
{
    ...
    p_audioBuffer->putAudio(&opusFrame);
