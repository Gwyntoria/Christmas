...
HI_S32 SAMPLE_COMM_VENC_Creat(VENC_CHN VencChn, PAYLOAD_TYPE_E enType,  PIC_SIZE_E enSize, SAMPLE_RC_E enRcMode, HI_U32  u32Profile, HI_BOOL bRcnRefShareBuf,VENC_GOP_ATTR_S *pstGopAttr)
{
    ...
    ******************************************/
    ...
    SAMPLE_PRT("SAMPLE_COMM_VENC_Creat: enType = %d, stPicSize.width = %d, stPicSize.height = %d, enRcMode = %d, u32Profile = %d, u32FrameRate = %d !\n", enType, stPicSize.u32Width, stPicSize.u32Height, enRcMode, u32Profile, u32FrameRate);
            ...
            if (SAMPLE_RC_CBR == enRcMode)
            {
                ...
                switch (enSize)
                {
                    case PIC_720P:
                        ...
                        break;
                    case PIC_1080P:
                        stH264Cbr.u32BitRate         = 1024 * 3;//1024 * 2 + 2048*u32FrameRate/30;
                        break;
                    case PIC_2592x1944:
...
HI_VOID* SAMPLE_COMM_QpmapSendFrameProc(HI_VOID* p)
{
    ...
    while(HI_TRUE == pstPara->bThreadStart)
    {
        ...
        for(VeChnCnt=0; VeChnCnt<pstPara->s32Cnt; VeChnCnt++)
        {
            ...
            stFrame[i].stUserRcInfo.bQpMapValid     = HI_TRUE;
