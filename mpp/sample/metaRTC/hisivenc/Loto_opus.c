...
int opus_dl_open(void **libHandle, char *libName)
{
    ...
    *libHandle = dlopen(libName, RTLD_LAZY | RTLD_LOCAL);
    ...
    if (NULL == *libHandle)
    {
        LOGE("dlopen %s failed, ret msg is %s! ", libName, dlerror());
        return -1;
    }
...
int opus_dl_dlsym(void **funcHandle, void *libHandle, char *funcName)
{
    ...
    *funcHandle = dlsym(libHandle, funcName);
    ...
    if (NULL == *funcHandle)
    {
        LOGE("dlsym %s failed, ret msg is %s! ", funcName, dlerror());
        return -1;
    }
...
int opus_dl_close(void *libHandle)
{
    ...
    {
        LOGE("dlclose lib failed, ret msg is %s! ", dlerror());
        return -1;
    }
                         ...
                         HI_BOOL bResampleEn, HI_VOID *pstAiVqeAttr, HI_U32 u32AiVqeType)
{
    ...
    if (ret)
    {
        LOGE("HI_MPI_AI_SetPubAttr(%d) failed with %#x", AiDevId, ret);
        return ret;
    }
    ...
    if (ret)
    {
        LOGE("HI_MPI_AI_Enable(%d) failed with %#x", AiDevId, ret);
        return ret;
    }
    ...
    for (i = 0; i < s32AiChnCnt >> pstAioAttr->enSoundmode; i++)
    {
        ...
        if (ret)
        {
            LOGE("HI_MPI_AI_EnableChn(%d,%d) failed with %#x", AiDevId, i, ret);
            return ret;
        }
        ...
        if (HI_TRUE == bResampleEn)
        {
            ...
            if (ret)
            {
                LOGE("HI_MPI_AI_EnableReSmp(%d,%d) failed with %#x", AiDevId, i, ret);
                return ret;
            }
        }
        ...
        if (NULL != pstAiVqeAttr)
        {
            ...
            if (ret)
            {
                LOGE("%s: SetAiVqe%d(%d,%d) failed with %#x", __FUNCTION__, u32AiVqeType, AiDevId, i, ret);
                return ret;
            }
            ...
            if (bAiVqe)
            {
                ...
                if (ret)
                {
                    LOGE("HI_MPI_AI_EnableVqe(%d,%d) failed with %#x", AiDevId, i, ret);
                    return ret;
                }
            }
        }
                        ...
                        HI_BOOL bResampleEn, HI_BOOL bVqeEn)
{
    ...
    for (i = 0; i < s32AiChnCnt; i++)
    {
        if (HI_TRUE == bResampleEn)
        {
            ...
            if (HI_SUCCESS != ret)
            {
                LOGE("[Func]:%s [Line]:%d [Info]:%s", __FUNCTION__, __LINE__, "failed");
                return ret;
            }
        }
...
HI_S32 LOTO_OPUS_ConfigAudioIn(AUDIO_SAMPLE_RATE_E enSample)
{
    ...
    if (fdAcodec < 0)
    {
        LOGE("Can't open Acodec,%s", ACODEC_FILE);
        return HI_FAILURE;
    }
    ...
    default:
        LOGE("Not support enSample:%d", enSample);
        ret = HI_FAILURE;
        break;
    }
...
HI_S32 LOTO_OPUS_InitEncoder(OpusEncoder **opusEncoder)
{
    ...
    *opusEncoder = g_stOpusEncFunc.opusEncoderCreate(stOpusAttr.samplingRate,
    ...
    {
        LOGE("Allocates and initializes an encoder state failed with %#x", ret);
        return -1;
    }
    ...
    if (ret != OPUS_OK)
    {
        LOGE("OPUS_SET_BITRATE failed with %#x", ret);
        return -1;
    }
    ...
    if (ret != OPUS_OK)
    {
        LOGE("OPUS_SET_EXPERT_FRAME_DURATION failed with %#x", ret);
        return -1;
    }
    ...
    if (ret != OPUS_OK)
    {
        LOGE("OPUS_SET_VBR failed with %#x", ret);
        return -1;
    }
    ...
    if (ret != OPUS_OK)
    {
        LOGE("OPUS_SET_FORCE_CHANNELS failed with %#x", ret);
        return -1;
    }
...
void *LOTO_OPUS_StartAenc(void *arg)
{
    ...
    LOGD("AiDevId = %d, AiChn = %d", AiDevId, AiChn);
    ...
    while (start)
    {
        ...
        if (FD_ISSET(ai_fd, &read_fds))
        {
            ...
            if (ret != HI_SUCCESS)
            {
                LOGE("HI_MPI_AI_GetFrame failed with %#x", ret);
                start = HI_FALSE;
                break;
            }
            ...
            do
            {
                ...
                // LOGD("pstFrm.u32Len = %d, pstFrm.enBitwidth = %d, point_num = %d", pstFrm.u32Len, pstFrm.enBitwidth, point_num);

                if (point_num != water_line)
                {
                    LOGE("Invalid point number %d for default opus encoder setting!", point_num);
                    start = HI_FALSE;
                    break;
                }
                ...
                if (ret < 0)
                {
                    LOGE("Opus encode failed with %#x", ret);
                    start = HI_FALSE;
                    break;
                }
                else
                ...
                // LOGD("opus_frame_size = %d", opus_data_len);
            ...
            if (ret != HI_SUCCESS)
            {
                LOGE("HI_MPI_AI_ReleaseFrame failed with %#x", ret);
                start = HI_FALSE;
                break;
            }
        }
