...
HI_S32 SAMPLE_VENC_SYS_Init(HI_U32 u32SupplementConfig,SAMPLE_SNS_TYPE_E  enSnsType)
{
    HI_S32 s32Ret;
    HI_U64 u64BlkSize;
    VB_CONFIG_S stVbConf; // VB configure
    ...
    // attributes of Video Buffer pools 
    ...
    stVbConf.u32MaxPoolCnt = 2; // VB pool count
...
HI_S32 SAMPLE_VENC_VI_Init( SAMPLE_VI_CONFIG_S *pstViConfig, HI_BOOL bLowDelay, HI_U32 u32SupplementConfig) {
    ...
    // pstViConfig->astViInfo[0].stDevInfo.enWDRMode          = WDR_MODE_2To1_LINE;
    ...
    // pstViConfig->astViInfo[0].stPipeInfo.aPipe[1]          = ViPipe1;
                                ...
                                SAMPLE_SNS_TYPE_E enSnsType)
{
    ...
    // stVpssGrpAttr.stNrAttr.enNrType             = VPSS_NR_TYPE_VIDEO_ENHANCE;
    ...
    for(i=0; i<VPSS_MAX_PHY_CHN_NUM; i++)
    {
        if(HI_TRUE == pabChnEnable[i])
        {
            ...
            // stVpssChnAttr[i].u32Width                     = 2560;
            // stVpssChnAttr[i].u32Height                    = 1440;
...
/**
 * @brief lowdelay H264e, Channel resolution adaptable with sensor
 * 
 * @return HI_S32 
 */
HI_S32 SAMPLE_VENC_LOW_DELAY(void)
{
    ...
    HI_S32 s32Ret;
    SIZE_S          stSize[2];
    PIC_SIZE_E      enSize[2]     = {PIC_1080P, PIC_720P}; // imx385, imx327, gc2053
    // PIC_SIZE_E      enSize[2]     = {PIC_2592x1944, PIC_720P}; // imx335
    ...
    // s32Ret = SAMPLE_VENC_VI_Init(&stViConfig, HI_FALSE, u32SupplementConfig);    // lowdelay == HI_FALSE --> VI_OFFLINE_VPSS_OFFLINE
    if(s32Ret != HI_SUCCESS)
    {
        SAMPLE_PRT("Init VI err for %#x!\n", s32Ret);
        return HI_FAILURE;
    ...
    ******************************************/
    ...
    /***encode h.264 **/
    s32Ret = SAMPLE_COMM_VENC_Start(VencChn[0], enPayLoad[1], enSize[0], enRcMode, u32Profile[0], bRcnRefShareBuf, &stGopAttr);
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("Venc Start failed for %#x!\n", s32Ret);
        goto EXIT_VI_VPSS_UNBIND;
