...
static HI_U32 g_au32STimeFps[ISP_MAX_PIPE_NUM] = {[0 ...(ISP_MAX_PIPE_NUM - 1)] = 30};
...
static HI_S32 cmos_get_ae_default(VI_PIPE ViPipe, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    if (pstSnsState->u8ImgMode == IMX335_5M_30FPS_12BIT_LINEAR_MODE) {
        ...
        pstSnsState->u32FLStd = u32Fll*U32MaxFps/DIV_0_TO_1_FLOAT(g_au32STimeFps[ViPipe]);
...
static HI_VOID cmos_fps_set(VI_PIPE ViPipe, HI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    pstAeSnsDft->f32Fps = f32Fps;
    g_au32STimeFps[ViPipe] = (HI_U32)f32Fps;
    pstAeSnsDft->u32FullLinesStd = pstSnsState->u32FLStd;
...
static HI_VOID cmos_inttime_update(VI_PIPE ViPipe, HI_U32 u32IntTime)
{
    ...
    static HI_U32 au32LastShortIntTime[ISP_MAX_PIPE_NUM] = {0};
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        if (bFirst[ViPipe]) { /* short exposure */
            ...
            u32SHR0 = (pstSnsState->au32FL[1] - u32LongIntTime) + ((pstSnsState->au32FL[1] - u32LongIntTime) % 4);
            if (au32LastShortIntTime[ViPipe] > au32ShortIntTime[ViPipe]) {
                u32delta = au32LastShortIntTime[ViPipe] - au32ShortIntTime[ViPipe];
                u32RHS1Limit = (pstSnsState->au32FL[1] - (g_astimx335State[ViPipe].u32BRL * 2) - 2);
                if (u32delta > u32RHS1Limit) {
                    au32ShortIntTime[ViPipe] = au32LastShortIntTime[ViPipe] - (u32RHS1Limit - (u32RHS1Limit % 8) + 8);
                }
            }

            u16ShortExpMod4 = au32ShortIntTime[ViPipe] % 4;
            u16ShortExpMod8 = au32ShortIntTime[ViPipe] % 8;

            if (u16ShortExpMod8 == 0) {
                u32SHR1 = 18;
                u32RHS1 = 18 + au32ShortIntTime[ViPipe];
            } else {
                if (u16ShortExpMod4 == u16ShortExpMod8) {
                    u32SHR1 = 18;
                    u32RHS1 = 18 + au32ShortIntTime[ViPipe] - u16ShortExpMod8;
                } else {
                    u32SHR1 = 22;
                    u32RHS1 = 22 + au32ShortIntTime[ViPipe] - u16ShortExpMod4;
                }
            }
            ...
            au32LastShortIntTime[ViPipe] = au32ShortIntTime[ViPipe];
        }
    } else { // Linear
...
static HI_VOID cmos_get_inttime_max(VI_PIPE ViPipe, HI_U16 u16ManRatioEnable, HI_U32 *au32Ratio, HI_U32 *au32IntTimeMax, HI_U32 *au32IntTimeMin, HI_U32 *pu32LFMaxIntTime)
{
    ...
    HI_U32 u32delta;
    static HI_U32 u32LastIntTimeMaxTmp[ISP_MAX_PIPE_NUM] = { [0 ...(ISP_MAX_PIPE_NUM - 1)] = 4 };
    ...
    if ((pstSnsState->enWDRMode == WDR_MODE_2To1_LINE)) {
        ...
        */
        if (genFSWDRMode[ViPipe] == ISP_FSWDR_LONG_FRAME_MODE) { /* long frame mode enable */
            ...
            u32LastIntTimeMaxTmp[ViPipe] = u32ShortTimeMinLimit;
            return;
            ...
            if (pstSnsState->au32WDRIntTime[0] == u32ShortTimeMinLimit && au32Ratio[0] == 0x40) {
                ...
                if (u32IntTimeMaxTmp > u32LastIntTimeMaxTmp[ViPipe]) {
                    u32delta = u32IntTimeMaxTmp - u32LastIntTimeMaxTmp[ViPipe];
                    if (u32delta > g_astimx335State[ViPipe].u32deltaRHS1) {
                        u32IntTimeMaxTmp = u32LastIntTimeMaxTmp[ViPipe] + (g_astimx335State[ViPipe].u32deltaRHS1 - (g_astimx335State[ViPipe].u32deltaRHS1 % 8));
                    } else {
                    }
                } else if (u32LastIntTimeMaxTmp[ViPipe] > u32IntTimeMaxTmp) {
                    u32delta = u32LastIntTimeMaxTmp[ViPipe] - u32IntTimeMaxTmp;
                    if (u32delta > g_astimx335State[ViPipe].u32deltaRHS1) {
                        u32IntTimeMaxTmp = u32LastIntTimeMaxTmp[ViPipe] - (g_astimx335State[ViPipe].u32deltaRHS1 - (g_astimx335State[ViPipe].u32deltaRHS1 % 8));
                    } else {
                    }
                ...
                u32LastIntTimeMaxTmp[ViPipe] = u32IntTimeMaxTmp;
            }
        } else {
            u32IntTimeMaxTmp0 = ((pstSnsState->au32FL[1] - 280 - pstSnsState->au32WDRIntTime[0]) * 0x40)  / DIV_0_TO_1(au32Ratio[0]);
            u32IntTimeMaxTmp = ((pstSnsState->au32FL[0] - 280) * 0x40)  / DIV_0_TO_1(au32Ratio[0] + 0x40);
            ...
            *pu32LFMaxIntTime = g_astimx335State[ViPipe].u32RHS1_MAX - 18;
            if (*pu32LFMaxIntTime > (3066 - 22)) {
                *pu32LFMaxIntTime = 3066 - 22;
            }

            u32IntTimeMaxTmp1 = pstSnsState->au32FL[0] - 22 - *pu32LFMaxIntTime - 164 - 4;

            if (u32IntTimeMaxTmp > u32IntTimeMaxTmp1) {
                u32IntTimeMaxTmp = u32IntTimeMaxTmp - 140;
                *pu32LFMaxIntTime = *pu32LFMaxIntTime - 24;
            }
            if (*pu32LFMaxIntTime > u32IntTimeMaxTmp) {
